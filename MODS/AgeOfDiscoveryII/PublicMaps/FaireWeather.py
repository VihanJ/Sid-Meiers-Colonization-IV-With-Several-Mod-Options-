##############################################################################
## File: FaireWeather.py version 1.03
...
## GLOBAL TUNING VARIABLES: Change these to customize the map results
        ...
        self.hmNumberOfPlates = int(float(self.hmWidth * self.hmHeight) * 0.0018)
        ...
        #Filter size for smoothing and also distance search. Must be odd number
        self.filterSize = 15
...
class HeightMap :
    ...
    def performTectonics(self):
        ...
        growthPlotList = list()
        plateList = list()
        maxDistance = math.sqrt(pow(float(mc.filterSize/2),2) + pow(float(mc.filterSize/2),2))
        #initialize maps
        for y in range(mc.hmHeight):
            for x in range(mc.hmWidth):
                i = GetHmIndex(x,y)
        ...
        while(len(growthPlotList) > 0):
            ...
            for direction in range(1,5,1):
                ...
                if self.plateMap[ii].plateID != plateID and self.plateMap[ii].plateID != 0:
                    ...
                    roomLeft = True
                    if PRand.random() < mc.plateGrowthChance:

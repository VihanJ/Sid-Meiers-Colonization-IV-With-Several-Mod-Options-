...
# < JTradeRoutes Mod Start 1/3 >
import CvAssignTradeRoutesToUnitPopup
assignTradeRoutesToUnitPopup = CvAssignTradeRoutesToUnitPopup.CvAssignTradeRoutesToUnitPopup()
def showAssignTradeRoutesToUnitPopup(iPlayer, iUnit, bTradeRoutesScreen, iTradeRoutesTab):
        assignTradeRoutesToUnitPopup.interfaceScreen(iPlayer, iUnit, bTradeRoutesScreen, iTradeRoutesTab)

import CvAssignUnitsToTradeRoutePopup
assignUnitsToTradeRoutePopup = CvAssignUnitsToTradeRoutePopup.CvAssignUnitsToTradeRoutePopup()
def showAssignUnitsToTradeRoutePopup(iPlayer, iTradeRouteID, bTradeRoutesScreen, iTradeRoutesTab):
        assignUnitsToTradeRoutePopup.interfaceScreen(iPlayer, iTradeRouteID, bTradeRoutesScreen, iTradeRoutesTab)

import CvTradeRoutesScreen
tradeRoutesScreen = CvTradeRoutesScreen.CvTradeRoutesScreen()
def showTradeRoutesScreen(iTab = 0):
	tradeRoutesScreen.interfaceScreen(iTab)
# < JTradeRoutes Mod End 1/3 >
...
# Pirates START
pirateScreen = CvEuropeScreen.CvEuropeScreen()
def showPirateScreen(argsList):
	if (-1 != CyGame().getActivePlayer()):
		pirateScreen.interfaceScreen()
# Pirates END
...
# Cutscene START
cutScene = CvCutsceneScreen.CvCutsceneScreen()
def showCutscene(argsList):
	cutScene.interfaceScreen(argsList[0], argsList[1])
# Cutscene END

# FoY START
FoY = CvFoYScreen.CvFoYScreen()
def showFoY(argsList):
	FoY.interfaceScreen(argsList[0], argsList[1])
# FoY END
...
HandleMouseoverMap = {  EUROPE_SCREEN : europeScreen,
				...
				# add new screens here
                                # < JTradeRoutes Mod Start 2/3 >
                                JTRADEROUTES_SCREEN: tradeRoutesScreen,
                                JTRADEROUTES_ASSIGN_TRADE_ROUTES_TO_UNIT_POPUP: assignTradeRoutesToUnitPopup,
                                JTRADEROUTES_ASSIGN_UNITS_TO_TRADE_ROUTE_POPUP: assignUnitsToTradeRoutePopup,
                                # < JTradeRoutes Mod End 2/3 >
# Achievements START
...
HandleInputMap = {  MAIN_INTERFACE : mainInterface,
				...
				# add new screens here
                                # < JTradeRoutes Mod Start 3/3 >
                                JTRADEROUTES_SCREEN: tradeRoutesScreen,
                                JTRADEROUTES_ASSIGN_TRADE_ROUTES_TO_UNIT_POPUP: assignTradeRoutesToUnitPopup,
                                JTRADEROUTES_ASSIGN_UNITS_TO_TRADE_ROUTE_POPUP: assignUnitsToTradeRoutePopup,
                                # < JTradeRoutes Mod End 3/3 >
# Achievements START
				ACHIEVE_ADVISOR : achieveAdvisor,
# Achievements END
				}

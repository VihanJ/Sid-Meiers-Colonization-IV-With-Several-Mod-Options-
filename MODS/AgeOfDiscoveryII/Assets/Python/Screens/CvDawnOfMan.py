...
class CvDawnOfMan:
	...
	def interfaceScreen(self):
		...
		if (gc.getPlayer(gc.getGame().getActivePlayer()).isPirates()):
			bodyString = localText.getText("TXT_KEY_ROYAL_CHARTER_BODY_2_PIRATE", (CyGameTextMgr().getTimeStr(gc.getGame().getGameTurn(), false), kingPlayer.getCivilizationShortDescriptionKey(), self.player.getNameKey()))
		else:
			bodyString = localText.getText("TXT_KEY_ROYAL_CHARTER_BODY_3", (CyGameTextMgr().getTimeStr(gc.getGame().getGameTurn(), false), kingPlayer.getCivilizationShortDescriptionKey(), self.player.getNameKey()))

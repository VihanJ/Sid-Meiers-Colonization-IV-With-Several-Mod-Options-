...
# < JTradeRoutes Mod Start 1/7 >
import CvScreensInterface
# < JTradeRoutes Mod End 1/7 >
...
# Achievements START
ACHIEVE_ADVISOR_SCREEN_MI = 4300
# Achievements END

# Advisor menu START
ADVISOR_MENU_PANEL = 4100
ADVISOR_MENU_PANEL_UP = True
# PatchMod: Advisor menu END

# < JTradeRoutes Mod Start 2/5 >
TRADE_ROUTES_ADVISOR_SCREEN = 4200
# < JTradeRoutes Mod End 2/5 >
...
# PatchMod: Advisor menu START
		screen.setImageButton("AdvisorButton", ArtFileMgr.getInterfaceArtInfo("INTERFACE_MAP_UNIT").getPath(), iBtnX, (TOP_CENTER_HUD_HEIGHT - self.ADVISOR_BUTTON_SIZE) / 2, self.ADVISOR_BUTTON_SIZE, self.ADVISOR_BUTTON_SIZE, WidgetTypes.WIDGET_GENERAL, ADVISOR_MENU_PANEL, -1 )
		screen.setImageShape("AdvisorButton", ImageShapes.IMAGE_SHAPE_ELLIPSE, -1)
		screen.setHitMargins("AdvisorButton", self.ADVISOR_BUTTON_SIZE / 6, self.ADVISOR_BUTTON_SIZE / 6)
		self.appendtoHideState(screen, "AdvisorButton", HIDE_TYPE_MAP, HIDE_LEVEL_HIDE)
		iBtnX += self.ADVISOR_BUTTON_SPACING
# PatchMod: Advisor menu END

# Pirates START
		pirPlayer = gc.getPlayer(gc.getGame().getActivePlayer())
		if (pirPlayer.isPirates()):
			screen.setImageButton("PirateScreenButton", ArtFileMgr.getInterfaceArtInfo("INTERFACE_EUROPE").getPath(), iBtnX, (TOP_CENTER_HUD_HEIGHT - self.ADVISOR_BUTTON_SIZE) / 2, self.ADVISOR_BUTTON_SIZE, self.ADVISOR_BUTTON_SIZE, WidgetTypes.WIDGET_ACTION, gc.getControlInfo(ControlTypes.CONTROL_PIRATE_SCREEN).getActionInfoIndex(), -1 )#WidgetTypes.WIDGET_GENERAL, PIRATES_COVE, -1 )
			screen.setImageShape("PirateScreenButton", ImageShapes.IMAGE_SHAPE_ELLIPSE, -1)
			screen.setHitMargins("PirateScreenButton", self.ADVISOR_BUTTON_SIZE / 6, self.ADVISOR_BUTTON_SIZE / 6)
			self.appendtoHideState(screen, "PirateScreenButton", HIDE_TYPE_MAP, HIDE_LEVEL_HIDE)
			iBtnX += self.ADVISOR_BUTTON_SPACING
		else:
...
# PatchMod: Advisor menu START
		if (ADVISOR_MENU_PANEL_UP):
			iBtnX = (xResolution / 2) + (TOP_CENTER_HUD_WIDTH / 2) - (self.ADVISOR_BUTTON_SIZE / 3)
			iBtnY = ((TOP_CENTER_HUD_HEIGHT - self.ADVISOR_BUTTON_SIZE) / 2) + (self.ADVISOR_BUTTON_SIZE / 3 * 2)
			...
			iBtnX += self.ADVISOR_BUTTON_SPACING
                        ...
                        # < JTradeRoutes Mod Start 3/7 >
                        sArtPath = gc.getCommandInfo(CommandTypes.COMMAND_CHOOSE_TRADE_ROUTES).getButton()#ArtFileMgr.getInterfaceArtInfo("INTERFACE_GENERAL_DIPLOMACY").getPath()#
                        screen.setImageButton("TradeRoutesButton", sArtPath, iBtnX, iBtnY, self.ADVISOR_BUTTON_SIZE, self.ADVISOR_BUTTON_SIZE, WidgetTypes.WIDGET_GENERAL, TRADE_ROUTES_ADVISOR_SCREEN, -1)
...
# Achievements START
			screen.setImageButton("AchieveAdvisorButton", ArtFileMgr.getInterfaceArtInfo("INTERFACE_ACHIEVE_BUTTON").getPath(), iBtnX, iBtnY, self.ADVISOR_BUTTON_SIZE, self.ADVISOR_BUTTON_SIZE, WidgetTypes.WIDGET_GENERAL, ACHIEVE_ADVISOR_SCREEN_MI, -1 )
...
# Achievements END
			...
			# < JTradeRoutes Mod Start 4/7 >
			elif (inputClass.getButtonType() == WidgetTypes.WIDGET_GENERAL and inputClass.getData1() == TRADE_ROUTES_ADVISOR_SCREEN):
                                CvScreensInterface.showTradeRoutesScreen(iTab = 0)
			# < JTradeRoutes Mod End 4/7 >		
...
# PatchMod: Advisor menu START
				...
				if not (gc.getGame().isGameMultiPlayer()):
					...
					screen.hide("AchieveAdvisorButton")
                                    else:
					ADVISOR_MENU_PANEL_UP = True
					...
					screen.show("AchieveAdvisorButton")
# Advisor menu END
		return 0
	...
	def getWidgetHelp( self, argsList ):
		...
		if eWidgetType == WidgetTypes.WIDGET_GENERAL:
			...
			# < JTradeRoutes Mod Start 7/7 >
			elif iData1 == TRADE_ROUTES_ADVISOR_SCREEN:
				return localText.getText("TXT_KEY_INTERFACE_TRADE_ROUTES_ADVISOR_TOGGLE", ("Alt", "T"));
			# < JTradeRoutes Mod End 5/5 >
# Achievements START
			elif iData1 == ACHIEVE_ADVISOR_SCREEN_MI:
				return localText.getText("TXT_KEY_INTERFACE_ACHIEVE_ADVISOR", ("F10", ""));
# Achievements END
# PatchMod: Advisor menu START
			elif iData1 == ADVISOR_MENU_PANEL:
				return localText.getText("TXT_KEY_INTERFACE_ADVISOR_TOGGLE", ());
# PatchMod: Advisor menu END

...
# Dale - AoD: Pirate Recruitment START
		if (iGameTurn >= 50):
			self.AoDPirateRecruitment()
# Dale - AoD: Pirate Recruitment END
...
# Dale - AoD: Pirate Recruitment START
	def AoDPirateRecruitment(self):
		iPirateUnit = CvUtil.findInfoTypeNum('UNIT_PIRATE')
		iPirateShipUnit = CvUtil.findInfoTypeNum('UNIT_PIRATE_SHIP')
		iPirateProfession = CvUtil.findInfoTypeNum('PROFESSION_SOLDIER')
		iColonistProfession = CvUtil.findInfoTypeNum('PROFESSION_COLONIST')
		for iPlayer in range(gc.getMAX_PLAYERS()):
			player = gc.getPlayer(iPlayer)
			if (player.isAlive() and player.isPirates()):
				(pCity, iter) = player.firstCity(false)
				while (pCity):
#					if (CyGame().getSorenRandNum(100, "Pirate Recruitment") < 10):
#						if (CyGame().getSorenRandNum(100, "Pirate Recruitment") < 50):
#							player.initUnit(iPirateUnit, iPirateProfession, pCity.getX(), pCity.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION, 0)
#						else:
#							player.initUnit(iPirateUnit, iColonistProfession, pCity.getX(), pCity.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION, 0)
					if (CyGame().getSorenRandNum(100, "Pirate Recruitment") < 1):
						if (pCity.isCoastal(1) and not player.isHuman()):
							player.initUnit(iPirateShipUnit, gc.getUnitInfo(iPirateShipUnit).getDefaultProfession(), pCity.getX(), pCity.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION, 0)
...
# EuropeScreen START	
	...
	def doEuropeScreenApply(self, playerID, userData, popupReturn):
		iMode, iUnit, iX, iY, iCityX, iCityY, iSellPrice = userData
		
		transport = gc.getPlayer(playerID).getUnit(iUnit)

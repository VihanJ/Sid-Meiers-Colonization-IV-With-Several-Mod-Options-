...
void CvCity::init(int iID, PlayerTypes eOwner, int iX, int iY, bool bBumpUnits)
{
	...
	// PatchMod: Give base buildings to first city START
	if (!GET_PLAYER(getOwnerINLINE()).isFoundedFirstCity() && !GET_PLAYER(getOwnerINLINE()).isNative())
	{
		setHasFreeBuilding(((BuildingTypes)0), true);
		setHasFreeBuilding(((BuildingTypes)4), true);
		setHasFreeBuilding(((BuildingTypes)8), true);
		setHasFreeBuilding(((BuildingTypes)12), true);
		setHasFreeBuilding(((BuildingTypes)16), true);
		setHasFreeBuilding(((BuildingTypes)20), true);
		setHasFreeBuilding(((BuildingTypes)24), true);
		setHasFreeBuilding(((BuildingTypes)28), true);
		setHasFreeBuilding(((BuildingTypes)32), true);
		setHasFreeBuilding(((BuildingTypes)45), true);
		setHasFreeBuilding(((BuildingTypes)49), true);
	}
	// PatchMod: Give base buildings to first city END
...
void CvCity::kill()
{
	...
	// PatchMod: Return mission success percent START
	...
	if (eOldPlayer != NO_PLAYER)
	{
		...
		GET_PLAYER(eOldPlayer).setMissionarySuccessPercent(GET_PLAYER(eOldPlayer).getMissionarySuccessPercent() / GC.getHandicapInfo(GC.getGameINLINE().getHandicapType()).getMissionFailureThresholdPercent() * 100);
	...
	// PatchMod: Return mission success percent END
...
void CvCity::doTask(TaskTypes eTask, int iData1, int iData2, bool bOption, bool bAlt, bool bShift, bool bCtrl)
{
	...
	case TASK_EDUCATESTART:
		educateStudentStart(iData1, (UnitTypes) iData2);
		break;
		// PatchMod: Education revamp END

		// PatchMod: Shortfall START
	case TASK_BUYBUILDSHORTFALL:
		doShortfall(1, iData1, iData2);
		break;

	case TASK_BUYUNITSHORTFALL:
		doShortfall(2, iData1, iData2);
		break;
		// PatchMod: Shortfall END
...
/*	for (int iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
	{
		if (kBuilding.getObsoleteBuildingClass(iI))
		{
			BuildingTypes eObsoleteBuilding = ((BuildingTypes)(GC.getCivilizationInfo(getCivilizationType()).getCivilizationBuildings(iI)));

			if (eObsoleteBuilding != NO_BUILDING)
			{
				if (isHasConceptualBuilding(eObsoleteBuilding))
				{
					return false;
				}
			}
	...
	if (kBuilding.isSingleInstance())
	{
		if (GET_PLAYER(getOwnerINLINE()).getBuildingClassCount((BuildingClassTypes)(GC.getBuildingInfo(eBuilding).getBuildingClassType())) > 0)
		{
			return false;
		}
	}
	...
	if (getFood() >= growthThreshold())
	{
		...
		else
		{
			...
			if (NO_UNIT != eUnit)
			{
				...
				pUnit->setToryValue(getRebelPercent());
				// PatchMod: Tories END
	...
	for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
	{
		...
		case YIELD_EDUCATION:
			{
				...
				for (uint i = 0; i < m_aPopulationUnits.size(); ++i)
				{
					...
					if (eProfession != NO_PROFESSION)
					{
						if (GC.getProfessionInfo(eProfession).getYieldProduced() == eYield)
						{
							// PatchMod: Education change START
							if (pLoopUnit->getLearningProfession() > 0)
							{
		...
		default:
			...
			if (GC.getYieldInfo(eYield).isCargo())
			{
				...
				if (iExcess > 0)
				{
					// PatchMod: More warehouse sales START
					int iLoss = std::max(getOverflowYieldSellPercent() * iExcess / 100, GC.getDefineINT("MIN_CITY_YIELD_DECAY"));
//					int iLoss = std::max(GC.getDefineINT("CITY_YIELD_DECAY_PERCENT") * iExcess / 100, GC.getDefineINT("MIN_CITY_YIELD_DECAY"));
					// PatchMod: More warehouse sales END
...
void CvCity::doRebelSentiment()
{
	...
	int toryvalue = 0;
	int iNew = 0;
	if (!GET_PLAYER(getOwnerINLINE()).isEurope() && !GET_PLAYER(getOwnerINLINE()).isNative() && !GET_PLAYER(getOwnerINLINE()).isPirates())
	{
...
int CvCity::getSpecialistTuition(UnitTypes eUnit) const
{
	...
	// PatchMod: Education revamp START
	if (GC.getUnitInfo(eUnit).getTeacherWeight() > getEducationBuildingLevel())
	{
		return -1;
	}
...
//	}
	...
	// PatchMod: Education revamp START
	iPrice *= GC.getUnitInfo(eUnit).getTeacherWeight();
	iPrice /= 4;
	// PatchMod: Education revamp END
...
{
	if (isHasBuilding((BuildingTypes)59) || isHasBuilding((BuildingTypes)60))
	{
		CvWString szBuffer;
		CvPlot* pPlot = plot();
						...
						if (pAdjacentPlot != NULL && iDamage == 0)
						{
							if (pAdjacentPlot->isWater())
							{
								...
								while (pUnitNode2)
								{
									...
									if (getTeam() != pLoopUnit2->getTeam() && GET_TEAM(getTeam()).isAtWar(pLoopUnit2->getTeam()) && !pLoopUnit2->isCargo() || GET_PLAYER(pLoopUnit2->getOwnerINLINE()).isPirates())
									{
...
// PatchMod: Education revamp START
int CvCity::getEducationBuildingLevel() const
{
	int value = 0;
	for(int i = 0; i < GC.getNumBuildingInfos(); i++)
	{
		if(isHasBuilding((BuildingTypes)i))
		{
			if (GC.getBuildingInfo((BuildingTypes)i).getEducationLevel() > value)
			{
				value = GC.getBuildingInfo((BuildingTypes)i).getEducationLevel();
			}
		}
	}
	return value;
...
bool CvCity::educateStudentStart(int iUnitId, UnitTypes eUnit)
{
	CvUnit* pUnit = getPopulationUnitById(iUnitId);
	if (pUnit == NULL)
	{
	...
	pUnit->setYieldStored(0);
	pUnit->setLearningProfession(eUnit);
	kPlayer.changeGold(-iPrice);
	return true;
}
// PatchMod: Education revamp END
...
// PatchMod: Shortfall START
void CvCity::doShortfall(int iType, int iData1, int iData2)
{
	if (iType == 1)
	{
//		GET_PLAYER(GC.getGameINLINE().getActivePlayer()).changeGold(-GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getYieldNeededPrice());
		GET_PLAYER(getOwnerINLINE()).changeGold(-iData2);
		for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
		{
			if (GC.getYieldInfo((YieldTypes) iYield).isCargo())
			{
				int iYieldNeeded = GET_PLAYER(getOwnerINLINE()).getYieldProductionNeeded((BuildingTypes) iData1, (YieldTypes) iYield);
				if (iYieldNeeded > getYieldStored((YieldTypes) iYield))
				{
					setYieldStored((YieldTypes)iYield, iYieldNeeded);
				}
			}
		}
	} else {
//		CvCity* pCity = GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCity(iData1);
		GET_PLAYER(GC.getGameINLINE().getActivePlayer()).changeGold(-GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getYieldNeededPrice());
		for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
		{
			if (GC.getYieldInfo((YieldTypes) iYield).isCargo())
			{
				int iYieldNeeded = GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getYieldProductionNeeded((UnitTypes) iData2, (YieldTypes) iYield);
				if (iYieldNeeded > getYieldStored((YieldTypes) iYield))
				{
					setYieldStored((YieldTypes)iYield, iYieldNeeded);
				}

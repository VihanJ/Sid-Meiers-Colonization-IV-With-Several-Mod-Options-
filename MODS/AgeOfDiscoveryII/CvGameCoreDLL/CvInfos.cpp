...
CvProfessionInfo::CvProfessionInfo() :
	// PatchMod: Cargo change START
	m_iProfCargoChange(0),
	// PatchMod: Cargo change END
	// PatchMod: Profession upgrade unit START
	m_iUpgradeUnit(-1),
	// PatchMod: Profession upgrade unit END
...
void CvProfessionInfo::read(FDataStreamBase* stream)
{
	...
	stream->Read(&uiFlag);		// flag for expansion
	// PatchMod: Cargo change START
	stream->Read(&m_iProfCargoChange);
	// PatchMod: Cargo change END
	// PatchMod: Profession upgrade unit START
	stream->Read(&m_iUpgradeUnit);
	// PatchMod: Profession upgrade unit END
...
void CvProfessionInfo::write(FDataStreamBase* stream)
{
	...
	stream->Write(uiFlag);		// flag for expansion
	// PatchMod: Cargo change START
	stream->Write(m_iProfCargoChange);
	// PatchMod: Cargo change END
	// PatchMod: Profession upgrade unit START
	stream->Write(m_iUpgradeUnit);
	// PatchMod: Profession upgrade unit END
...
bool CvProfessionInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// PatchMod: Cargo change START
	pXML->GetChildXmlValByName(&m_iProfCargoChange, "iProfCargoChange");
	// PatchMod: Cargo change END
	// PatchMod: Profession upgrade unit START
	pXML->GetChildXmlValByName(&m_iUpgradeUnit, "iUpgradeUnit");
	// PatchMod: Profession upgrade unit END
	m_aYieldEquipments.clear();
	int *aiYieldAmounts;
...
// PatchMod: Impact Europe price START
m_bImpactEuropePrice(false),
// PatchMod: Impact Europe price END
// PatchMod: Education revamp START
m_iEducationLevel(0),
// PatchMod: Education revamp END
...
// PatchMod: Zuuls tags START
m_abObsoleteBuildingClass(NULL),
m_bSingleInstance(false),
// PatchMod: Zuuls tags END
m_aiYieldCost(NULL),
...
CvBuildingInfo::~CvBuildingInfo()
{
	...
	// PatchMod: Zuuls tags START
	SAFE_DELETE_ARRAY(m_abObsoleteBuildingClass);
	// PatchMod: Zuuls tags END
}
...
void CvBuildingInfo::read(FDataStreamBase* stream)
{
	...
	// PatchMod: Impact Europe price START
	stream->Read(&m_bImpactEuropePrice);
	// PatchMod: Impact Europe price END

	// PatchMod: Zuuls tags START
	stream->Read(&m_bSingleInstance);
	SAFE_DELETE_ARRAY(m_abObsoleteBuildingClass);
	m_abObsoleteBuildingClass = new bool[GC.getNumBuildingClassInfos()];
	stream->Read(GC.getNumBuildingClassInfos(), m_abObsoleteBuildingClass);
	// PatchMod: Zuuls tags END

	// PatchMod: Education revamp START
	stream->Read(&m_iEducationLevel);
	// PatchMod: Education revamp END
...
void CvBuildingInfo::write(FDataStreamBase* stream)
{
	...
	// PatchMod: Impact Europe price START
	stream->Write(m_bImpactEuropePrice);
	// PatchMod: Impact Europe price END

	// PatchMod: Zuuls tags START
	stream->Write(m_bSingleInstance);
	stream->Write(GC.getNumBuildingClassInfos(), m_abObsoleteBuildingClass);
	// PatchMod: Zuuls tags END

	// PatchMod: Education revamp START
	stream->Write(m_iEducationLevel);
	// PatchMod: Education revamp END
...
bool CvBuildingInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// PatchMod: Impact Europe price START
	pXML->GetChildXmlValByName(&m_bImpactEuropePrice, "bImpactEuropePrice");
	// PatchMod: Impact Europe price END
	...
	pXML->SetVariableListTagPair(&m_aiYieldCost, "YieldCosts", NUM_YIELD_TYPES, 0);
	// PatchMod: Zuuls tags START
	pXML->GetChildXmlValByName(&m_bSingleInstance, "bSingleInstance");
	pXML->SetVariableListTagPair(&m_abObsoleteBuildingClass, "ObsoleteBuildingClasses", GC.getNumBuildingClassInfos(), false);
	// PatchMod: Zuuls tags END
	// PatchMod: Education revamp START
	pXML->GetChildXmlValByName(&m_iEducationLevel, "iEducationLevel");
	// PatchMod: Education revamp END
	return true;
}
...
m_bEurope(false),
m_bNative(false),
// Dale - AoD: Pirate Civ START
m_bPirates(false),
// Dale - AoD: Pirate Civ END
// Dale - AoD: Natives START
m_bAsianNative(false),
m_bNAmericanNative(false),
m_bSAmericanNative(false),
m_bAfricanNative(false),
m_bPacificNative(false),
// Dale - AoD: Natives END
// PatchMod: Is from west START
m_bFromWest(false),
// PatchMod: Is from west END
...
void CvCivilizationInfo::read(FDataStreamBase* stream)
{
	...
	// Dale - AoD: Pirate Civ START
	stream->Read(&m_bPirates);
	// Dale - AoD: Pirate Civ END
	// Dale - AoD: Natives START
	stream->Read(&m_bAsianNative);
	stream->Read(&m_bNAmericanNative);
	stream->Read(&m_bSAmericanNative);
	stream->Read(&m_bAfricanNative);
	stream->Read(&m_bPacificNative);
	// Dale - AoD: Natives END
	stream->ReadString(m_szArtDefineTag);
	// Dale - AoD: Rev Flags START
	stream->ReadString(m_szRevArtDefineTag);
	// Dale - AoD: Rev Flags END
	// PatchMod: Is from west START
	stream->Read(&m_bFromWest);
	// PatchMod: Is from west END
...
void CvCivilizationInfo::write(FDataStreamBase* stream)
{
	...
	// Dale - AoD: Pirate Civ START
	stream->Write(m_bPirates);
	// Dale - AoD: Pirate Civ END
	// Dale - AoD: Natives START
	stream->Write(m_bAsianNative);
	stream->Write(m_bNAmericanNative);
	stream->Write(m_bSAmericanNative);
	stream->Write(m_bAfricanNative);
	stream->Write(m_bPacificNative);
	// Dale - AoD: Natives END
	stream->WriteString(m_szArtDefineTag);
	// Dale - AoD: Rev Flags START
	stream->WriteString(m_szRevArtDefineTag);
	// Dale - AoD: Rev Flags END
	// PatchMod: Is from west START
	stream->Write(m_bFromWest);
	// PatchMod: Is from west END
...
bool CvCivilizationInfo::read(CvXMLLoadUtility* pXML)
{
	...
	setArtDefineTag(szTextVal);
	// Dale - AoD: Rev Flags START
	pXML->GetChildXmlValByName(szTextVal, "RevArtDefineTag");
	setRevArtDefineTag(szTextVal);
	// Dale - AoD: Rev Flags END
	...
	// Dale - AoD: Pirate Civ START
	pXML->GetChildXmlValByName(&m_bPirates, "bPirates");
	// Dale - AoD: Pirate Civ END
	// Dale - AoD: Is from west START
	pXML->GetChildXmlValByName(&m_bFromWest, "bFromWest");
	// Dale - AoD: Is from west END
	// Dale - AoD: Natives START
	pXML->GetChildXmlValByName(&m_bAsianNative, "bAsianNative");
	pXML->GetChildXmlValByName(&m_bNAmericanNative, "bNAmericanNative");
	pXML->GetChildXmlValByName(&m_bSAmericanNative, "bSAmericanNative");
	pXML->GetChildXmlValByName(&m_bAfricanNative, "bAfricanNative");
	pXML->GetChildXmlValByName(&m_bPacificNative, "bPacificNative");
	// Dale - AoD: Natives END
...
m_iTotalGoodsTraded(0),
m_iTotalProductionRate(0),
m_iTotalPiratePlunder(0),
...
bool CvVictoryInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// PatchMod: Victorys START
	pXML->GetChildXmlValByName(&m_iTotalGoodsTraded, "iTotalGoodsTraded");
	pXML->GetChildXmlValByName(&m_iTotalProductionRate, "iTotalProductionRate");
	pXML->GetChildXmlValByName(&m_iTotalPiratePlunder, "iTotalPiratePlunder");
	// PatchMod: Victorys END
...
m_iKingUnitIncreaseCap(0),
// PatchMod: REF Reduction END
// PatchMod: Harder REF START
m_iREFStrength(0),
// PatchMod: Harder REF END
// PatchMod: King DoW START
m_iKingDowPercent(0),
// PatchMod: King DoW END
m_iNumGoodies(0),
...
void CvHandicapInfo::read(FDataStreamBase* stream)
{
	...
	stream->Read(&m_iNumAIImmigrants);
	// PatchMod: AI immigration boost END
	...
	// PatchMod: REF Reduction START
	stream->Read(&m_iKingUnitIncreaseCap);
	// PatchMod: REF Reduction END
	// PatchMod: Harder REF START
	stream->Read(&m_iREFStrength);
	// PatchMod: Harder REF END
	// PatchMod: King DoW START
	stream->Read(&m_iKingDowPercent);
	// PatchMod: King DoW END
...
void CvHandicapInfo::write(FDataStreamBase* stream)
{
	...
	stream->Write(m_iNumAIImmigrants);
	// PatchMod: AI immigration boost END
	...
	// PatchMod: REF Reduction START
	stream->Write(m_iKingUnitIncreaseCap);
	// PatchMod: REF Reduction END
	// PatchMod: Harder REF START
	stream->Write(m_iREFStrength);
	// PatchMod: Harder REF END
	// PatchMod: King DoW START
	stream->Write(m_iKingDowPercent);
	// PatchMod: King DoW END
...
bool CvHandicapInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(&m_iNumAIImmigrants, "iNumAIImmigrants");
	// PatchMod: AI immigration boost END
	...
	// PatchMod: REF Reduction START
	pXML->GetChildXmlValByName(&m_iKingUnitIncreaseCap, "iKingUnitIncreaseCap");
	// PatchMod: REF Reduction END
	// PatchMod: Harder REF START
	pXML->GetChildXmlValByName(&m_iREFStrength, "iREFStrength");
	// PatchMod: Harder REF END
	// PatchMod: King DoW START
	pXML->GetChildXmlValByName(&m_iKingDowPercent, "iKingDowPercent");
	// PatchMod: King DoW END
...
// PatchMod: START
m_bAlwaysApplyYieldIncreases(false),
// PatchMod: END
m_iAdvancedStartCost(0),
m_iAdvancedStartCostIncrease(0),
m_iTilesPerGoody(0),
...
void CvImprovementInfo::read(FDataStreamBase* stream)
{
	...
	stream->Read(&uiFlag);		// flag for expansion
	// PatchMod: START
	stream->Read(&m_bAlwaysApplyYieldIncreases);
	// PatchMod: END
	stream->Read(&m_iAdvancedStartCost);
	stream->Read(&m_iAdvancedStartCostIncrease);
...
void CvImprovementInfo::write(FDataStreamBase* stream)
{
	...
	stream->Write(uiFlag);		// flag for expansion
	// PatchMod: START
	stream->Write(m_bAlwaysApplyYieldIncreases);
	// PatchMod: END
	stream->Write(m_iAdvancedStartCost);
	stream->Write(m_iAdvancedStartCostIncrease);
...
bool CvImprovementInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// PatchMod: START
	pXML->GetChildXmlValByName(&m_bAlwaysApplyYieldIncreases, "bAlwaysApplyYieldIncreases");
	// PatchMod: END
	pXML->GetChildXmlValByName(szTextVal, "ArtDefineTag");
	setArtDefineTag(szTextVal);
...
CvEuropeInfo::CvEuropeInfo() :
	...
	m_iTripLength(0),
	// PatchMod: Western nations START
	m_iWestTripLength(0),
	// PatchMod: Western nations END
...
// PatchMod: Western nations START
int CvEuropeInfo::getWestTripLength() const
{
	return m_iWestTripLength;
}
// PatchMod: Western nations END
...
bool CvEuropeInfo::read(CvXMLLoadUtility* pXML)
{
	...
	// PatchMod: Western nations START
	pXML->GetChildXmlValByName(&m_iWestTripLength, "iWestTripLength");
	// PatchMod: Western nations END
...
// Dale - AoD: Pirate Civ START
bool CvCivilizationInfo::isPirates() const
{
	return m_bPirates;
}
// Dale - AoD: Pirate Civ END
...
// Dale - AoD: Is from west START
bool CvCivilizationInfo::isFromWest() const
{
//	return true;
	return m_bFromWest;
}
...
// Dale - AoD: Natives START
bool CvCivilizationInfo::isAsianNative() const
{
	return m_bAsianNative;
}

bool CvCivilizationInfo::isNAmericanNative() const
{
	return m_bNAmericanNative;
}

bool CvCivilizationInfo::isSAmericanNative() const
{
	return m_bSAmericanNative;
}

bool CvCivilizationInfo::isAfricanNative() const
{
	return m_bAfricanNative;
}

bool CvCivilizationInfo::isPacificNative() const
{
	return m_bPacificNative;
}
// Dale - AoD: Natives END

// Dale - AoD: Rev Flags START
const char* CvCivilizationInfo::getRevArtDefineTag() const
{
	return m_szRevArtDefineTag;
}

void CvCivilizationInfo::setRevArtDefineTag(const char* szVal)
{
	m_szRevArtDefineTag = szVal;
}
// Dale - AoD: Rev Flags END
...
// PatchMod: Unit freedom START
int CvUnitInfo::getTurnsToFreedom()
{
...
// PatchMod: Impact Europe price START
bool CvBuildingInfo::isImpactEuropePrice() const
{
	return m_bImpactEuropePrice;
}
// PatchMod: Impact Europe price END
...
// PatchMod: Cutscene START
CvCutsceneInfo::CvCutsceneInfo() :
	m_iPictureX(0),
	m_iPictureY(0)
{
}
...
bool CvCutsceneInfo::read(CvXMLLoadUtility* pXML)
{
	if (!CvInfoBase::read(pXML))
	{
		return false;
...
const wchar* CvCutsceneInfo::getDescription() const
{
	return m_szDescriptionKey;
}

const wchar* CvCutsceneInfo::getHeaderTextKey() const
{
	return m_szHeaderTextKey;
}

const char* CvCutsceneInfo::getPictureFileName() const
{
	return m_szPictureFileName;
}

int CvCutsceneInfo::getPictureX()
{
	return m_iPictureX;
}

int CvCutsceneInfo::getPictureY()
{
	return m_iPictureY;
}
// PatchMod: Cutscene END

// PatchMod: Victorys START
int CvVictoryInfo::getTotalGoodsTraded() const
{
	return m_iTotalGoodsTraded;
}
...
int CvVictoryInfo::getTotalPiratePlunder() const
{
	return m_iTotalPiratePlunder;
}

void CvVictoryInfo::setTotalGoodsTraded(int iValue)
{
	m_iTotalGoodsTraded = iValue;
}
...
void CvVictoryInfo::setTotalPiratePlunder(int iValue)
{
	m_iTotalPiratePlunder = iValue;
}
// PatchMod: Victorys END

// PatchMod: Cargo change START
int CvProfessionInfo::getProfCargoChange() const
{
	return m_iProfCargoChange;
}
// PatchMod: Cargo change END

// PatchMod: Zuuls tags START
bool CvBuildingInfo::isSingleInstance() const
{
	return m_bSingleInstance;
}

bool CvBuildingInfo::getObsoleteBuildingClass(int i) const
{
	FAssertMsg(i < GC.getNumBuildingClassInfos(), "Index out of bounds");
	FAssertMsg(i > -1, "Index out of bounds");
	return m_abObsoleteBuildingClass ? m_abObsoleteBuildingClass[i] : false;
}
// PatchMod: Zuuls tags END

// PatchMod: Profession upgrade unit START
int CvProfessionInfo::getUpgradeUnit() const
{
	return m_iUpgradeUnit;
}
// PatchMod: Profession upgrade unit END

// PatchMod: Education revamp START
int CvBuildingInfo::getEducationLevel() const
{
	return m_iEducationLevel;
}
// PatchMod: Education revamp END

// PatchMod: REF Reduction START
int CvHandicapInfo::getKingUnitIncreaseCap() const
{
	return m_iKingUnitIncreaseCap;
}
// PatchMod: REF Reduction END

// PatchMod: Harder REF START
int CvHandicapInfo::getREFStrength() const
{
	return m_iREFStrength;
}
// PatchMod: Harder REF END

// PatchMod: King DoW START
int CvHandicapInfo::getKingDowPercent() const
{
	return m_iKingDowPercent;
}
// PatchMod: King DoW END
...
int CvHandicapInfo::getNumAIImmigrants() const
{
	return m_iNumAIImmigrants;
}
// PatchMod: AI immigration boost END

// PatchMod: START
bool CvImprovementInfo::getAlwaysApplyYieldIncreases() const
{
	return m_bAlwaysApplyYieldIncreases;
}
// PatchMod: END

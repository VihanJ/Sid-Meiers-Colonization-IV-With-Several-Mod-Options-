...
bool CyGame::isWBAsianNative()
{
	return (NULL != m_pGame ? m_pGame->isWBAsianNative() : -1);
}

void CyGame::setWBAsianNative(bool bValue)
{
	if (m_pGame)
	{
		m_pGame->setWBAsianNative(bValue);
	}
}

bool CyGame::isWBNAmericanNative()
{
	return (NULL != m_pGame ? m_pGame->isWBNAmericanNative() : -1);
}

void CyGame::setWBNAmericanNative(bool bValue)
{
	if (m_pGame)
	{
		m_pGame->setWBNAmericanNative(bValue);
	}
}

bool CyGame::isWBSAmericanNative()
{
	return (NULL != m_pGame ? m_pGame->isWBSAmericanNative() : -1);
}

void CyGame::setWBSAmericanNative(bool bValue)
{
	if (m_pGame)
	{
		m_pGame->setWBSAmericanNative(bValue);
	}
}

bool CyGame::isWBAfricanNative()
{
	return (NULL != m_pGame ? m_pGame->isWBAfricanNative() : -1);
}

void CyGame::setWBAfricanNative(bool bValue)
{
	if (m_pGame)
	{
		m_pGame->setWBAfricanNative(bValue);
	}
}

bool CyGame::isWBPacificNative()
{
	return (NULL != m_pGame ? m_pGame->isWBPacificNative() : -1);
}

void CyGame::setWBPacificNative(bool bValue)
{
	if (m_pGame)
	{
		m_pGame->setWBPacificNative(bValue);
	}
}

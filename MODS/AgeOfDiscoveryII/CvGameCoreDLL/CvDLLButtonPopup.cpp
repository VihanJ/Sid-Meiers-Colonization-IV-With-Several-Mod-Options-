...
void CvDLLButtonPopup::OnOkClicked(CvPopup* pPopup, PopupReturn *pPopupReturn, CvPopupInfo &info)
{
	...
	case BUTTONPOPUP_CHOOSEPRODUCTION:
		...
		if (pPopupReturn->getButtonClicked() == -1)
		{
			int item = GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getBuildItemNumber();
			if (item >= GC.getNumUnitInfos())
			{
				BuildingTypes eBuilding = (BuildingTypes)(item - GC.getNumUnitInfos());
				gDLL->sendDoTask(info.getData1(), TASK_BUYBUILDSHORTFALL, eBuilding, GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getYieldNeededPrice(), false, false, false, false);
//				for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
//				{
//					if (GC.getYieldInfo((YieldTypes) iYield).isCargo())
//					{
//						int iYieldNeeded = GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getYieldProductionNeeded(eBuilding, (YieldTypes) iYield);
//						if (iYieldNeeded > pCity->getYieldStored((YieldTypes) iYield))
//						{
//							pCity->setYieldStored((YieldTypes)iYield, iYieldNeeded);
//						}
//					}
//				}
			...
			{
				UnitTypes eUnit = (UnitTypes)item;
				gDLL->sendDoTask(info.getData1(), TASK_BUYUNITSHORTFALL, eUnit, -1, false, false, false, false);
//				for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
//				{
//					if (GC.getYieldInfo((YieldTypes) iYield).isCargo())
//					{
//						int iYieldNeeded = GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getYieldProductionNeeded(eUnit, (YieldTypes) iYield);
//						if (iYieldNeeded > pCity->getYieldStored((YieldTypes) iYield))
//						{
//							pCity->setYieldStored((YieldTypes)iYield, iYieldNeeded);
//						}
...
//				}
			...
			GET_PLAYER(GC.getGameINLINE().getActivePlayer()).setYieldNeededPrice(0);
			GET_PLAYER(GC.getGameINLINE().getActivePlayer()).setBuildItemNumber(0);
			break;
		}
		GET_PLAYER(GC.getGameINLINE().getActivePlayer()).setYieldNeededPrice(0);
		GET_PLAYER(GC.getGameINLINE().getActivePlayer()).setBuildItemNumber(0);
	...
	case BUTTONPOPUP_CHOOSE_EDUCATION:
		if (pPopupReturn->getButtonClicked() == GC.getNumUnitInfos())
		{
			CvCity* pTempCity = GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCity(info.getData1());
			if (pTempCity != NULL)
			{
				pTempCity->setCitizenAutomated(info.getData2());
...
bool CvDLLButtonPopup::launchButtonPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	...
	case BUTTONPOPUP_CUTSCENE:
		bLaunched = launchCutscenePopup(pPopup, info);
		break;
	// PatchMod: Cutscene END
	// PatchMod: FoY START
	case BUTTONPOPUP_FOY:
		bLaunched = launchFoYPopup(pPopup, info);
		break;
	// PatchMod: FoY END
	default:
		FAssert(false);
...
bool CvDLLButtonPopup::launchProductionPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	...
	case (ORDER_TRAIN):
		...
		// PatchMod: Allow yield shortage purchase on build START
		GET_PLAYER(pCity->getOwnerINLINE()).setBuildItemNumber(info.getData3());
		// PatchMod: Allow yield shortage purchase on build END
		break;
	case (ORDER_CONSTRUCT):
		eConstructBuilding = (BuildingTypes)info.getData3();
		// PatchMod: Allow yield shortage purchase on build START
		GET_PLAYER(pCity->getOwnerINLINE()).setBuildItemNumber(GC.getNumUnitInfos() + info.getData3());
		// PatchMod: Allow yield shortage purchase on build END
		break;
	default:
		break;
	...
	if (eTrainUnit != NO_UNIT)
	{
		...
		{
			// PatchMod: Allow yield shortage purchase on build START
			GET_PLAYER(pCity->getOwnerINLINE()).setYieldNeededPrice(0);
			// PatchMod: Allow yield shortage purchase on build END
		...
		else
		{
			...
			for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
			{
				if (GC.getYieldInfo((YieldTypes) iYield).isCargo())
				{
					...
					{
						// PatchMod: Allow yield shortage purchase on build START
						int iYieldPrice = (iYieldNeeded - pCity->getYieldStored((YieldTypes) iYield)) * GET_PLAYER(pCity->getOwnerINLINE()).getYieldBuyPrice((YieldTypes)iYield) * GC.getHurryInfo((HurryTypes)0).getYieldCostEuropePercent() / 100;
						GET_PLAYER(pCity->getOwnerINLINE()).setYieldNeededPrice(GET_PLAYER(pCity->getOwnerINLINE()).getYieldNeededPrice() + iYieldPrice);
						// PatchMod: Allow yield shortage purchase on build END
						if (!szRequires.empty())
						{
							szRequires += L", ";
	...
	// PatchMod: Allow yield shortage purchase on build START
	if (!bFinish && GET_PLAYER(pCity->getOwnerINLINE()).canTradeWithEurope() && GET_PLAYER(pCity->getOwnerINLINE()).getGold() >= GET_PLAYER(pCity->getOwnerINLINE()).getYieldNeededPrice())
	{
		if (GET_PLAYER(pCity->getOwnerINLINE()).getYieldNeededPrice() > 0)
		{
			szBuffer = gDLL->getText("TXT_KEY_POPUP_HURRY_SHORTAGE", GET_PLAYER(pCity->getOwnerINLINE()).getYieldNeededPrice());
			gDLL->getInterfaceIFace()->popupAddGenericButton(pPopup, szBuffer, ARTFILEMGR.getInterfaceArtInfo("INTERFACE_EUROPE_PURCHASE_UNIT")->getPath(), -1, WIDGET_GENERAL, -1, -1, true, POPUP_LAYOUT_STRETCH, DLL_FONT_LEFT_JUSTIFY);
		}
	}
	// PatchMod: Allow yield shortage purchase on build END
...
bool CvDLLButtonPopup::launchEducationPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	...
	if ((YieldTypes)GC.getProfessionInfo(pUnit->getProfession()).getYieldProduced() != YIELD_EDUCATION)
	{
		return false;
	}
...
//		if (iNumUnits == 1)
//		{
//			gDLL->sendDoTask(info.getData1(), TASK_EDUCATESTART, info.getData2(), eLastUnit, false, false, false, false);
//		}
//		return false;
//	}
	...
	if ((GC.getGameINLINE().getElapsedGameTurns() == 0) && !(GC.getGameINLINE().isGameMultiPlayer()) && !(GC.getInitCore().getWBMapScript()))
	{
		...
		for (int i = 0; bShow && i < MAX_TEAMS; i++)
		{
			for (int j = i+1; bShow && j < MAX_TEAMS; j++)
			{
				...
				{
					// PatchMod: Allow regenerate map START
//					bShow = false;
					// PatchMod: Allow regenerate map END
				}
			}
		}
...
// PatchMod: Cutscene START
bool CvDLLButtonPopup::launchCutscenePopup(CvPopup* pPopup, CvPopupInfo &info)
{
	CyArgsList argsList;
	argsList.add(info.getData1());
	argsList.add(info.getData2());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "showCutscene", argsList.makeFunctionArgs());
	return (false);
}
// PatchMod: Cutscene END

// PatchMod: FoY START
bool CvDLLButtonPopup::launchFoYPopup(CvPopup* pPopup, CvPopupInfo &info)
{
	CyArgsList argsList;
	argsList.add(info.getData1());
	argsList.add(info.getData2());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "showFoY", argsList.makeFunctionArgs());
	return (false);
}
// PatchMod: FoY END

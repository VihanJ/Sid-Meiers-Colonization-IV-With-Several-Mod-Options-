...
CvGame::CvGame()
{
	...
	// PatchMod: Victories START
	iVictoriesSet = 0;
	// PatchMod: Victories END
	// PatchMod: FoY START
	bFoY = false;
	// PatchMod: FoY END
...
void CvGame::setInitialItems(bool bScenario)
{
	...
	for (int i = 0; i < MAX_PLAYERS; ++i)
	{
		...
		{
			if (!((GC.getGame().isWBAsianNative() && GC.getCivilizationInfo(GET_PLAYER((PlayerTypes)i).getCivilizationType()).isAsianNative()) || (GC.getGame().isWBNAmericanNative() && GC.getCivilizationInfo(GET_PLAYER((PlayerTypes)i).getCivilizationType()).isNAmericanNative()) || (GC.getGame().isWBSAmericanNative() && GC.getCivilizationInfo(GET_PLAYER((PlayerTypes)i).getCivilizationType()).isSAmericanNative()) || (GC.getGame().isWBAfricanNative() && GC.getCivilizationInfo(GET_PLAYER((PlayerTypes)i).getCivilizationType()).isAfricanNative()) || (GC.getGame().isWBPacificNative() && GC.getCivilizationInfo(GET_PLAYER((PlayerTypes)i).getCivilizationType()).isPacificNative())))
			{
				int iLeader = 0;
				int iLeaderStep = 0;
				int iCiv = 0;
				...
				{
					if ((GC.getGame().isWBAsianNative() && GC.getCivilizationInfo((CivilizationTypes)j).isAsianNative()) || (GC.getGame().isWBNAmericanNative() && GC.getCivilizationInfo((CivilizationTypes)j).isNAmericanNative()) || (GC.getGame().isWBSAmericanNative() && GC.getCivilizationInfo((CivilizationTypes)j).isSAmericanNative()) || (GC.getGame().isWBAfricanNative() && GC.getCivilizationInfo((CivilizationTypes)j).isAfricanNative()) || (GC.getGame().isWBPacificNative() && GC.getCivilizationInfo((CivilizationTypes)j).isPacificNative()))
					{
						iCiv++;
					}
				}
				iCivStep = GC.getGameINLINE().getSorenRandNum(iCiv, "Pick Civ");
				iCiv = -1;
				for (j = 0; j < GC.getNumCivilizationInfos(); j++)
				{
					if ((GC.getGame().isWBAsianNative() && GC.getCivilizationInfo((CivilizationTypes)j).isAsianNative()) || (GC.getGame().isWBNAmericanNative() && GC.getCivilizationInfo((CivilizationTypes)j).isNAmericanNative()) || (GC.getGame().isWBSAmericanNative() && GC.getCivilizationInfo((CivilizationTypes)j).isSAmericanNative()) || (GC.getGame().isWBAfricanNative() && GC.getCivilizationInfo((CivilizationTypes)j).isAfricanNative()) || (GC.getGame().isWBPacificNative() && GC.getCivilizationInfo((CivilizationTypes)j).isPacificNative()))
					{
						iCiv = j;
					...
					if (iCivStep < 0)
					{
						j = GC.getNumCivilizationInfos();
					}
				}
				if (iCiv >= 0)
				{
					iLeaderStep = GC.getGameINLINE().getSorenRandNum(GC.getCivilizationInfo((CivilizationTypes)iCiv).getNumLeaders(), "Pick Leader");
					for (j = 0; j < GC.getNumLeaderHeadInfos(); j++)
					{
						if (GC.getCivilizationInfo((CivilizationTypes)iCiv).isLeaders(j))
						{
							iLeader = j;
							iLeaderStep--;
						}
...
void CvGame::reset(HandicapTypes eHandicap, bool bConstructorCall)
{
	...
	// Dale - AoD: Natives START
	m_bWBAsianNative = false;
	m_bWBNAmericanNative = true;
	m_bWBSAmericanNative = true;
	m_bWBAfricanNative = false;
	m_bWBPacificNative = false;
	// Dale - AoD: Natives END
...
void CvGame::assignStartingPlots()
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if (GC.getCivilizationInfo(GET_PLAYER((PlayerTypes)iI).getCivilizationType()).isFromWest())
			{
				CvPlot* pWestPlot = GET_PLAYER((PlayerTypes)iI).getStartingPlot();
				CvPlot* pLooperPlot = NULL;
				CvPlot* pBestPlot = pWestPlot;
				...
				{
					gDLL->callUpdater();
					pLooperPlot = GC.getMapINLINE().plotByIndexINLINE(iJ);
					if (pLooperPlot->isWater())
					{
						if (pLooperPlot->getY_INLINE() > 5 && pLooperPlot->getY_INLINE() < (GC.getMapINLINE().getGridHeightINLINE() - 5))
						{
							if (pLooperPlot->getX_INLINE() < GC.getMapINLINE().getGridWidthINLINE() / 3)
							{
								if (pLooperPlot->isEurope())
								{
//									iRandNum = (getSorenRandNum(100, "") + (2 * pLooperPlot->getX_INLINE()));
									iRandNum = (getSorenRandNum(100, "") + (100 * pLooperPlot->getX_INLINE()));
									for (int iK = 0; iK < MAX_PLAYERS; iK++)
									{
										if (GET_PLAYER((PlayerTypes)iK).isAlive())
										{
											pTestPlot = GET_PLAYER((PlayerTypes)iK).getStartingPlot();
											if (plotDistance(pTestPlot->getX_INLINE(), pTestPlot->getY_INLINE(), pLooperPlot->getX_INLINE(), pLooperPlot->getY_INLINE()) < 10)
											{
												iRandNum = 0;
											}
										}
									}
									if (iRandNum > iCounter)
									{
										iCounter = iRandNum;
										pBestPlot = pLooperPlot;
									}
								}
							}
						}
...
bool CvGame::testVictory(VictoryTypes eVictory, TeamTypes eTeam, bool* pbEndScore) const
{
	...
	if (bValid)
	{
		if (GC.getVictoryInfo(eVictory).getTotalGoodsTraded() > 0)
		{
			if (GC.getVictoryInfo(eVictory).getTotalGoodsTraded() > GET_TEAM(eTeam).getTotalGoodsTraded())
			{
				bValid = false;
			}
		}
	...
	if (bValid)
	{
		if (GC.getVictoryInfo(eVictory).getTotalPiratePlunder() > 0)
		{
			if (GC.getVictoryInfo(eVictory).getTotalPiratePlunder() > GET_TEAM(eTeam).getTotalPiratePlunder())
			{
				bValid = false;
			}
		}
...
void CvGame::testVictory()
{
	...
	for (int iJ = 0; iJ < GC.getNumVictoryInfos(); iJ++)
	{
		...
		if (GC.getVictoryInfo(eVictory).getTotalGoodsTraded() > 0)
		{
			if (iVictoriesSet == 0)
			{
				GC.getVictoryInfo(eVictory).setTotalGoodsTraded(GC.getVictoryInfo(eVictory).getTotalGoodsTraded() * GC.getGameSpeedInfo(GC.getInitCore().getGameSpeed()).getGrowthPercent() / 100);
			}
		}
		if (GC.getVictoryInfo(eVictory).getTotalProductionRate() > 0)
		{
		...
		if (GC.getVictoryInfo(eVictory).getTotalPiratePlunder() > 0)
		{
			if (iVictoriesSet == 0)
			{
				GC.getVictoryInfo(eVictory).setTotalPiratePlunder(GC.getVictoryInfo(eVictory).getTotalPiratePlunder() * GC.getGameSpeedInfo(GC.getInitCore().getGameSpeed()).getGrowthPercent() / 100);
			}
		}
	}
	iVictoriesSet = 1;
...
void CvGame::read(FDataStreamBase* pStream)
{
	...
	// Dale - AoD: Natives START
	pStream->Read(&m_bWBAsianNative);
	pStream->Read(&m_bWBNAmericanNative);
	pStream->Read(&m_bWBSAmericanNative);
	pStream->Read(&m_bWBAfricanNative);
	pStream->Read(&m_bWBPacificNative);
	// Dale - AoD: Natives END
...
void CvGame::write(FDataStreamBase* pStream)
{
	...
	// Dale - AoD: Natives START
	pStream->Write(m_bWBAsianNative);
	pStream->Write(m_bWBNAmericanNative);
	pStream->Write(m_bWBSAmericanNative);
	pStream->Write(m_bWBAfricanNative);
	pStream->Write(m_bWBPacificNative);
	// Dale - AoD: Natives END
...
// Dale - AoD: Natives START
bool CvGame::isWBAsianNative() const
{
	return m_bWBAsianNative;
}

void CvGame::setWBAsianNative(bool bValue)
{
	m_bWBAsianNative = bValue;
}

bool CvGame::isWBNAmericanNative() const
{
	return m_bWBNAmericanNative;
}

void CvGame::setWBNAmericanNative(bool bValue)
{
	m_bWBNAmericanNative = bValue;
}

bool CvGame::isWBSAmericanNative() const
{
	return m_bWBSAmericanNative;
}

void CvGame::setWBSAmericanNative(bool bValue)
{
	m_bWBSAmericanNative = bValue;
}

bool CvGame::isWBAfricanNative() const
{
	return m_bWBAfricanNative;
}

void CvGame::setWBAfricanNative(bool bValue)
{
	m_bWBAfricanNative = bValue;
}

bool CvGame::isWBPacificNative() const
{
	return m_bWBPacificNative;
}

void CvGame::setWBPacificNative(bool bValue)
{
	m_bWBPacificNative = bValue;
}
// Dale - AoD: Natives END

// PatchMod: Cutscene START
void CvGame::doCutscene()
{
	int iCutscene = getSorenRandNum(GC.getNumCutsceneInfos(), NULL);
	int iRandTurn = getSorenRandNum(50, NULL);
	if (GC.getGameINLINE().getGameTurn() == 0 && !isGameMultiPlayer())
	{
		CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_CUTSCENE);
		pInfo->setData1(0);
		pInfo->setData2(1);
		gDLL->getInterfaceIFace()->addPopup(pInfo);
	}
...
//		{
//			CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_CUTSCENE);
//			pInfo->setData1((CutsceneTypes)iCutscene);
//			pInfo->setData2(0);
//			gDLL->getInterfaceIFace()->addPopup(pInfo);
//		}
//	}
...
{
	int iCutscene = 0;
	if (GC.getGameINLINE().getGameTurn() > 0 && !bFoY)
	{
		CvPopupInfo* pInfo = new CvPopupInfo(BUTTONPOPUP_FOY);
		pInfo->setData1((CutsceneTypes)iCutscene);
		pInfo->setData2(0);
		gDLL->getInterfaceIFace()->addPopup(pInfo);
		for (int iPlayer = 0; iPlayer < MAX_PLAYERS; iPlayer++)
		{
			CvPlayer& kChildPlayer = GET_PLAYER((PlayerTypes) iPlayer);
			if(kChildPlayer.isAlive() && (!kChildPlayer.isEurope()) && (!kChildPlayer.isNative()))
			{
				int iRand = getSorenRandNum(10, NULL) + 5;
				for (int iCount = 0; iCount < iRand; iCount++)
				{
					if (!kChildPlayer.isHuman())
					{
						kChildPlayer.doAIImmigrant(1);
					} else {
						kChildPlayer.doFoYImmigrant(1);
					}
				}
			}
		}

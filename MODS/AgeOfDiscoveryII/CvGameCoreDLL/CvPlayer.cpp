...
CvPlayer::CvPlayer()
{
	...
	m_aaiYieldStoredHistory = NULL;
	m_aaiYieldRateHistory = NULL;
...
void CvPlayer::uninit()
{
	...
	// < JTradeRoutes Mod Start >
	if (m_aaiYieldStoredHistory != NULL)
	{
		for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
		{
			SAFE_DELETE_ARRAY(m_aaiYieldStoredHistory[iI]);
		}
		SAFE_DELETE_ARRAY(m_aaiYieldStoredHistory);
	}
	if (m_aaiYieldRateHistory != NULL)
	{
		for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
		{
			SAFE_DELETE_ARRAY(m_aaiYieldRateHistory[iI]);
		}
		SAFE_DELETE_ARRAY(m_aaiYieldRateHistory);
	}
	// < JTradeRoutes Mod End >
...
void CvPlayer::reset(PlayerTypes eID, bool bConstructorCall)
{
	...
	// PatchMod: Allow yield shortage purchase on build START
	m_iYieldNeededPrice = 0;
	m_iBuildItemNumber = -1;
	// PatchMod: Allow yield shoratge purchase on build END

	// PatchMod: Victorys START
	m_iTotalGoodsTradedVictory = 0;
	...
	if (!bConstructorCall)
	{
		...
		// < JTradeRoutes Mod Start >
		FAssertMsg(m_aaiYieldStoredHistory==NULL, "about to leak memory, /*JTradeRoutes Mod*/ CvPlayer::m_aaiYieldStoredHistory");
		m_aaiYieldStoredHistory = new int*[1000];
		for (iI = 0; iI < 1000; iI++)
		{
			m_aaiYieldStoredHistory[iI] = new int[NUM_YIELD_TYPES];
			for (int iYield = 0; iYield < NUM_YIELD_TYPES; iYield++)
			{
				m_aaiYieldStoredHistory[iI][iYield] = 0;
			}
		}
		FAssertMsg(m_aaiYieldRateHistory==NULL, "about to leak memory, /*JTradeRoutes Mod*/ CvPlayer::m_aaiYieldStoredHistory");
		m_aaiYieldRateHistory = new int*[1000];
		for (iI = 0; iI < 1000; iI++)
		{
			m_aaiYieldRateHistory[iI] = new int[NUM_YIELD_TYPES];
			for (int iYield = 0; iYield < NUM_YIELD_TYPES; iYield++)
			{
				m_aaiYieldRateHistory[iI][iYield] = 0;
			}
		}
		// < JTradeRoutes Mod End >
...
void CvPlayer::doTurn()
{
	...
	// PatchMod: AI population booster START
	if (GC.getGameINLINE().getGameTurn() > 1)
	{
		for (int iCount = 0; iCount < GC.getHandicapInfo(getHandicapType()).getNumAIImmigrants(); iCount++)
		{
			if (!isHuman() && !isEurope() && !isNative())
			{
				if (GC.getGameINLINE().getSorenRandNum(100, "AI") < GC.getHandicapInfo(getHandicapType()).getAIImmigration())
				{
					doAIImmigrant(1);
				}
//				if (GC.getGameINLINE().getSorenRandNum(100, "AI") < GC.getHandicapInfo(getHandicapType()).getAIImmigration())
...
//				}
	...
	if (GC.getGameINLINE().getGameTurn() > 20)
	{
		if (!isHuman() && !isEurope() && !isNative())
		{
			if (GC.getGameINLINE().getSorenRandNum(100, "AI") < (GC.getHandicapInfo(getHandicapType()).getAIImmigration() / 8))
			{
				buyEuropeUnit((UnitTypes)34, 0); // galleon
			}
		}
	}
	// PatchMod: AI population booster END
	...
	for (int iI = 0; iI < NUM_YIELD_TYPES; ++iI)
	{
	    for (pLoopCity = firstCity(&iLoop); pLoopCity != NULL; pLoopCity = nextCity(&iLoop))
        {
            updateYieldStoredHistory(GC.getGameINLINE().getGameTurn(), (YieldTypes) iI, pLoopCity->getYieldStored((YieldTypes) iI));
        }
	}
	for (int iI = 0; iI < NUM_YIELD_TYPES; ++iI)
	{
	    for (pLoopCity = firstCity(&iLoop); pLoopCity != NULL; pLoopCity = nextCity(&iLoop))
        {
            updateYieldRateHistory(GC.getGameINLINE().getGameTurn(), (YieldTypes) iI, pLoopCity->getYieldRate((YieldTypes) iI));
        }
	}
	// < JTradeRoutes Mod End >
...
//			if (eCheapestShip != NO_UNIT && getGold() < getEuropeUnitBuyPrice(eCheapestShip))
			// PatchMod: Give ship to pilgrim regardless of gold END
			{
				...
				if (!bHasShip)
				{
					...
					// PatchMod: Give ship to pilgrim regardless of gold END
					// PatchMod: No pirates first 30 turns START
					if (isPirates() && GC.getGameINLINE().getGameTurn() < 30)
					{
						iNewTaxRate = iOldTaxRate;
					}
					// PatchMod: No pirates first 30 turns END
...
void CvPlayer::handleDiploEvent(DiploEventTypes eDiploEvent, PlayerTypes ePlayer, int iData1, int iData2)
{
	...
	case DIPLOEVENT_ACCEPT_TAX_RATE:
		...
		int iLoop;
		for (CvUnit* pToryUnit = firstUnit(&iLoop); pToryUnit != NULL; pToryUnit = nextUnit(&iLoop))
		{
			if(pToryUnit != NULL)
			{
			    pToryUnit->processToryChange(-2);
			}
		}
		// PatchMod: Tories END
		break;
...
void CvPlayer::doGoody(CvPlot* pPlot, CvUnit* pUnit)
{
	...
	else
	{
		...
		if (!isNative())
		{
			...
			// PatchMod: FoY START
			int iRand = GC.getGameINLINE().getSorenRandNum(100, "FoY");
			if (iRand == 1)
			{
				GC.getGameINLINE().doFoY();
			}
			// PatchMod: FoY END
		}
	}
...
// < JTradeRoutes Mod Start >
void CvPlayer::updateYieldStoredHistory(int iTurn, YieldTypes eYield, int iYieldStored)
{
    if (eYield == NO_YIELD || iTurn < 0 || iYieldStored < 0)
    {
        return;
    }
    m_aaiYieldStoredHistory[iTurn][(int) eYield] += iYieldStored;
}

int CvPlayer::getYieldStoredHistory(int iTurn, YieldTypes eYield) const
{
	if (eYield != NO_YIELD || iTurn < 0)
	{
        return m_aaiYieldStoredHistory[iTurn][(int)eYield];
	}
	return 0;
}

void CvPlayer::updateYieldRateHistory(int iTurn, YieldTypes eYield, int iYieldStored)
{
    if (eYield == NO_YIELD || iTurn < 0 || iYieldStored < 0)
    {
        return;
    }
    m_aaiYieldRateHistory[iTurn][(int) eYield] += iYieldStored;
}

int CvPlayer::getYieldRateHistory(int iTurn, YieldTypes eYield) const
{
	if (eYield != NO_YIELD || iTurn < 0)
	{
        return m_aaiYieldRateHistory[iTurn][(int)eYield];
	}
	return 0;
}
// < JTradeRoutes Mod End >
		...
		if (getBellsStored() >= revolutionEuropeUnitThreshold() && iBellsRate > GC.getCivilizationInfo(getCivilizationType()).getFreeYields(YIELD_BELLS))
		{
			...
			if (NO_PLAYER != getParent())
			{
				...
				if (iNumFreeUnits > 0)
				{
					...
					if (iNumUnits > GC.getHandicapInfo(getHandicapType()).getKingUnitIncreaseCap())
					{
						if (isHuman())
						{
                            iNumUnits = GC.getHandicapInfo(getHandicapType()).getKingUnitIncreaseCap();
						} else {
                            iNumUnits = GC.getHandicapInfo(getHandicapType()).getKingUnitIncreaseCap() / 4;
						}
					}
...
void CvPlayer::read(FDataStreamBase* pStream)
{
	...
	// PatchMod: Victorys START
	pStream->Read(&m_iTotalGoodsTradedVictory);
	pStream->Read(&m_iTotalPiratePlunder);
	// PatchMod: Victorys END
	...
	// < JTradeRoutes Mod Start >
	{
        for (iI = 0; iI < 1000; iI++)
        {
            pStream->Read(NUM_YIELD_TYPES, m_aaiYieldStoredHistory[iI]);
        }
	}
	{
        for (iI = 0; iI < 1000; iI++)
        {
            pStream->Read(NUM_YIELD_TYPES, m_aaiYieldRateHistory[iI]);
        }
	}
	// < JTradeRoutes Mod End >
...
void CvPlayer::write(FDataStreamBase* pStream)
{
	...
	// PatchMod: Victorys START
	pStream->Write(m_iTotalGoodsTradedVictory);
	pStream->Write(m_iTotalPiratePlunder);
	// PatchMod: Victorys END
	...
	// < JTradeRoutes Mod Start >
	{
        for (iI = 0; iI < 1000; iI++)
        {
            pStream->Write(NUM_YIELD_TYPES, m_aaiYieldStoredHistory[iI]);
        }
	}
	{
        for (iI = 0; iI < 1000; iI++)
        {
            pStream->Write(NUM_YIELD_TYPES, m_aaiYieldRateHistory[iI]);
        }
	}
	// < JTradeRoutes Mod End >
...
void CvPlayer::changeYieldTradedTotal(YieldTypes eYield, int iChange)
{
	...
	// PatchMod: Victorys START
	setTotalGoodsTradedVictory(getTotalGoodsTradedVictory() + iChange);
	// PatchMod: Victorys END
}
...
void CvPlayer::doPrices()
{
	...
	//do tax rate change
	if (!GC.getEraInfo(getCurrentEra()).isRevolution() && !isPirates())
	{
		PlayerTypes eParent = getParent();
		if (eParent != NO_PLAYER)
		{
...
void CvPlayer::hurry(HurryTypes eHurry, int iIndex)
{
	...
	else
	{
		...
		if (kHurry.getGoldPerCross() > 0)
		{
			...
			// PatchMod: Larger AI empires START
			if (!isHuman())
			{
				for (int i = 0; i < 3; i++)
				{
			...
			// PatchMod: Larger AI empires END
		}
	}
}
...
bool CvPlayer::isPirates() const
{
	...
	return GC.getCivilizationInfo(eCivilizationType).isPirates();
}
...
// PatchMod: Allow yield shortage purchase on build START
void CvPlayer::setYieldNeededPrice(int iValue)
{
	m_iYieldNeededPrice = iValue;
}

int CvPlayer::getYieldNeededPrice()
{
	return m_iYieldNeededPrice;
}

void CvPlayer::setBuildItemNumber(int iValue)
{
	m_iBuildItemNumber = iValue;
}

int CvPlayer::getBuildItemNumber()
{
	return m_iBuildItemNumber;
}
// PatchMod: Allow yield shortage purchase on build END

// PatchMod: Victorys START
void CvPlayer::setTotalGoodsTradedVictory(int iValue)
{
	m_iTotalGoodsTradedVictory = iValue;
}

int CvPlayer::getTotalGoodsTradedVictory()
{
	return m_iTotalGoodsTradedVictory;
}

void CvPlayer::setTotalPiratePlunder(int iValue)
{
	m_iTotalPiratePlunder = iValue;
}

int CvPlayer::getTotalPiratePlunder()
{
...
void CvPlayer::doFoYImmigrant(int iIndex)
{
	if (getParent() != NO_PLAYER && canTradeWithEurope())
	{	
		UnitTypes eBestUnit = getDocksNextUnit(iIndex);

...
void CvUnit::init(int iID, UnitTypes eUnit, ProfessionTypes eProfession, UnitAITypes eUnitAI, PlayerTypes eOwner, int iX, int iY, DirectionTypes eFacingDirection, int iYieldStored)
{
	...
	if(m_eFacingDirection == NO_DIRECTION)
	{
		...
		{
			// PatchMod: From west START
//			m_eFacingDirection = (DirectionTypes) GC.getDefineINT("WATER_UNIT_FACING_DIRECTION");
			if (GC.getCivilizationInfo(getCivilizationType()).isFromWest())
			{
				m_eFacingDirection = DIRECTION_EAST;
			} else {
				m_eFacingDirection = DIRECTION_WEST;
			}
			// PatchMod: From west END
		}
		else
		{
	...
	CvPlot* pTempPlot = GC.getMapINLINE().plotINLINE(iX, iY);
	if (pTempPlot != NULL)
	{
		if (pTempPlot->isCity())
		{
			CvCity* pTempCity = pTempPlot->getPlotCity();
			setToryValue(pTempCity->getRebelPercent());
		}
	}
	// PatchMod: Tories END

	// PatchMod: Harder REF START
	if (GET_PLAYER(eOwner).isEurope())
	{
		if (getDomainType() == DOMAIN_LAND)
		{
			m_iBaseCombat = (m_iBaseCombat * GC.getHandicapInfo(GC.getGameINLINE().getHandicapType()).getREFStrength()) / 100;
		}
	}
...
void CvUnit::convert(CvUnit* pUnit, bool bKill)
{
	...
	// PatchMod: Tories START
	setToryValue(pUnit->getToryValue());
	// PatchMod: Tories END
...
bool CvUnit::canTradeYield(const CvPlot* pPlot) const
{
	...
	// PatchMod: Can't trade before speaking with chief START
	if (pCity->isNative() && !pCity->isScoutVisited(getTeam()))
	{
		return false;
	}
	// PatchMod: Can't trade before speaking with chief END
...
bool CvUnit::canCrossOcean(const CvPlot* pPlot, UnitTravelStates eNewState) const
{
	...
	// PatchMod: Send MoWs to New World START
	if (eNewState == UNIT_TRAVEL_STATE_TO_EUROPE && GET_PLAYER(getOwnerINLINE()).isEurope())
	{
		if (cargoSpace() < 1)
		{
            return false;
	...
	// PatchMod: Send MoWs to New World END
...
void CvUnit::crossOcean(UnitTravelStates eNewState)
{
	...
	// PatchMod: Western nations travel time START
	if (GC.getCivilizationInfo(GET_PLAYER(getOwnerINLINE()).getCivilizationType()).isFromWest())
	{
		iTravelTime = GC.getEuropeInfo(plot()->getEurope()).getWestTripLength();
	}
	// PatchMod: Western nations travel time END
...
bool CvUnit::canSpeakWithChief(CvPlot* pPlot) const
{
	...
	// PatchMod: Allow ships speak to chief START
//	if (eProfession == NO_PROFESSION)
//	{
//		return false;
//	}
	// PatchMod: Allow ships speak to chief END
...
int CvUnit::cargoSpace() const
{
	...
	if (getProfession() != NO_PROFESSION)
	{
        return m_iCargoCapacity + GC.getProfessionInfo(getProfession()).getProfCargoChange();
	} else {
...
void CvUnit::setProfession(ProfessionTypes eProfession, bool bForce)
{
	...
	if (eProfession != NO_PROFESSION)
	{
		...
		{
			if (GC.getProfessionInfo(eProfession).getYieldProduced() == YIELD_EDUCATION)
			{
				CvCity* pCity = plot()->getPlotCity();
				if (pCity == NULL)
				{
...
bool CvUnit::canHaveProfession(ProfessionTypes eProfession, bool bBumpOther, const CvPlot* pPlot) const
{
	...
	if (kNewProfession.getYieldProduced() == YIELD_EDUCATION)
	{
		...
		CvCity* aCity = GET_PLAYER(getOwnerINLINE()).getPopulationUnitCity(getID());
		if (aCity == NULL)
		{
			aCity = plot()->getPlotCity();
		}
		int iCounter = 0;
		if (aCity != NULL)
		{
			for (int iI = 0; iI < GC.getNumUnitInfos(); iI++)
			{
				int iPrice = aCity->getSpecialistTuition((UnitTypes) iI);
				if (iPrice > 0 && iPrice <= GET_PLAYER(getOwnerINLINE()).getGold())
				{
					iCounter++;
				}
			}
...
void CvUnit::processProfessionStats(ProfessionTypes eProfession, int iChange)
{
	if (iChange != 0)
	{
		...
		if (eProfession != NO_PROFESSION)
		{
			...
			if (GET_PLAYER(getOwnerINLINE()).isEurope())
			{
				int iCombatChange = iChange * (kProfession.getCombatChange() + kOwner.getProfessionCombatChange(eProfession));
				iCombatChange = (iCombatChange * GC.getHandicapInfo(GC.getGameINLINE().getHandicapType()).getREFStrength()) / 100;
				setBaseCombatStr(baseCombatStr() + iCombatChange);
			} else {
...
{
	// Dale - AoD: Pirate Raid START
	CvWString szBuffer;
	if (GET_PLAYER(eNewValue).isPirates())
	{
		int iCaptureGold = 0;
		int iLoserGold = GET_PLAYER(getOwnerINLINE()).getGold();
		if (iLoserGold >= 100)
		{
			iCaptureGold = GC.getGameINLINE().getSorenRandNum(50, "Pirate Gold") + 50;
		} else {
			iCaptureGold = GC.getGameINLINE().getSorenRandNum(iLoserGold, "Pirate Gold");
		}
		GET_PLAYER(eNewValue).changeGold(iCaptureGold);
		GET_PLAYER(eNewValue).setTotalPiratePlunder(GET_PLAYER(eNewValue).getTotalPiratePlunder() + iCaptureGold);
		szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_GOLD_CAPTURE", iCaptureGold);
		gDLL->getInterfaceIFace()->addMessage(eNewValue, true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_THEIR_WITHDRAWL", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), plot()->getX_INLINE(), plot()->getY_INLINE());
		GET_PLAYER(getOwnerINLINE()).changeGold(-iCaptureGold);
		szBuffer = gDLL->getText("TXT_KEY_MISC_HIM_GOLD_CAPTURE", iCaptureGold);
		gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_THEIR_WITHDRAWL", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), plot()->getX_INLINE(), plot()->getY_INLINE());
	}
	// Dale - AoD: Pirate Raid END
			...
			if (!m_pUnitInfo->isTreasure() && getYieldStored() > 0)
			{
				...
				if (pCity != NULL)
				{
					if (getYieldStored() >= pCity->educationThreshold())
					{
						...
						{
							// PatchMod: Education revamp START
//							gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), "Got here!", "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), NULL, NULL, true, true);
							pCity->educateStudent(getID(), (UnitTypes)getLearningProfession());
...
{
	// PatchMod: Pirates START
//	if (!isNative())
	if (!isNative() || !GET_PLAYER(getOwnerINLINE()).isPirates())
	// PatchMod: Pirates END
	{
		return false;
	}
...
//	}
	...
	for (int iYield = 0; iYield < NUM_YIELD_TYPES; ++iYield)
	{
		...
		if (aYields[iYield] > 0)
		{
			...
			// PatchMod: Pirates START
			GET_PLAYER(getOwnerINLINE()).setTotalPiratePlunder(GET_PLAYER(getOwnerINLINE()).getTotalPiratePlunder() + aYields[iYield]);
			// PatchMod: Pirates END
...
//	}
	...
	if (pHomeCity != NULL)
	{
		...
		// PatchMod: Pirates START
		GET_PLAYER(getOwnerINLINE()).setTotalPiratePlunder(GET_PLAYER(getOwnerINLINE()).getTotalPiratePlunder() + iYieldsStolen);
		// PatchMod: Pirates END
	}
...
// PatchMod: Profession upgrade unit START
int CvUnit::getTurnsInJob()
{
	return m_iTurnsInJob;
}

void CvUnit::setTurnsInJob(int iValue)
{
	m_iTurnsInJob = iValue;
}

bool CvUnit::processProfessionUpgrade()
{
	CvPlot* buttonPlot = NULL;
	if (isNative())
	{
		return false;
	}
	if (m_pUnitInfo->getTeacherWeight() > 0 || m_pUnitInfo->getStudentWeight() <= 0)
	{
		return false;
	}
	if (getProfession() == NO_PROFESSION)
	...
	if ((UnitTypes)GC.getProfessionInfo(getProfession()).getUpgradeUnit() < 0)
	{
		return false;
	}
	CvCity* pCity = plot()->getPlotCity();
	if (pCity != NULL)
	{
		if ((YieldTypes)GC.getProfessionInfo(getProfession()).getYieldProduced() == NO_YIELD)
		{
			return false;
		}
		if ((YieldTypes)GC.getProfessionInfo(getProfession()).getYieldConsumed() != NO_YIELD)
		{
			if (pCity->getRawYieldProduced((YieldTypes)GC.getProfessionInfo(getProfession()).getYieldConsumed()) < 1)
			{
				return false;
			}
		}
	}
	int iTurns = GC.getGameINLINE().getSorenRandNum(5, "");
	setTurnsInJob(getTurnsInJob() + iTurns + 1);
	if (getTurnsInJob() < 200)
	{
		return false;
	}
	if (pCity != NULL)
	{
		...
		{
			setTurnsInJob(getTurnsInJob() - iTurns);
			return false;
		}
		buttonPlot = pCity->plot();
	} else {
		buttonPlot = plot();
	}
	CvUnit* pLearnUnit = GET_PLAYER(getOwnerINLINE()).initUnit((UnitTypes)GC.getProfessionInfo(getProfession()).getUpgradeUnit(), NO_PROFESSION, getX_INLINE(), getY_INLINE(), AI_getUnitAIType());
	pLearnUnit->convert(this, false);
	pLearnUnit->setProfession(getProfession(), true);
	CvWString szProfession = GC.getProfessionInfo(getProfession()).getTextKeyWide();
	CvWString szBuffer = gDLL->getText("TXT_KEY_PROFESSION_UPGRADE", szProfession.GetCString(), pLearnUnit->getNameKey());
	gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), buttonPlot->getX_INLINE(), buttonPlot->getY_INLINE(), true, true);
	kill(true);
	return true;
}
// PatchMod: Profession upgrade unit END
...
// PatchMod: Unit freedom START
int CvUnit::getTurnsToFreedom() const
{
	return m_iTurnsToFreedom;
}

void CvUnit::setTurnsToFreedom(int iValue)
{
	m_iTurnsToFreedom = iValue;
}

bool CvUnit::processUnitFreedom()
{
	if (getTurnsToFreedom() < 1)
	{
		return false;
	}
	...
	pLearnUnit->convert(this, false);
	pLearnUnit->setProfession(getProfession(), true);
	CvWString szBuffer = gDLL->getText("TXT_KEY_UNIT_FREEDOM", getNameKey(), pLearnUnit->getNameKey());
...
bool CvUnit::processSchoolTest()
{
	...
	if (GC.getProfessionInfo(getProfession()).getYieldProduced() != YIELD_EDUCATION)
	{
		return false;
	}
	CvCity* pCity = plot()->getPlotCity();
...
// PatchMod: Tories START
int CvUnit::getToryValue() const
{
	return m_iToryValue;
}

void CvUnit::setToryValue(int iValue)
{
	m_iToryValue = iValue;
	if (m_iToryValue > 100)
	{
...
void CvUnit::changeToryValue(int iValue)
{
	m_iToryValue += iValue;
	if (m_iToryValue > 100)
	{
...
bool CvUnit::processToryChange(int iValue, bool bApply)
{
//	int score = (100 / GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getGrowthPercent()) * 100;
//	if (bApply)
//	{
//		if (GC.getGameINLINE().getSorenRandNum(100, "") >= score)
//		{
//			return false;
//		}
//	}
	int score = (100 * GC.getHandicapInfo(GC.getGameINLINE().getHandicapType()).getAIGrowthPercent()) / 100;
	if (bApply && isHuman())
	{
		if (GC.getGameINLINE().getSorenRandNum(100, "") >= score)
		{
	...
	{
		iValue = 0 - iValue;
		changeToryValue(0 - GC.getGameINLINE().getSorenRandNum((iValue + 1), ""));
	} else {
        changeToryValue(GC.getGameINLINE().getSorenRandNum((iValue + 1), ""));
	}
	return true;
}
// PatchMod: Tories END

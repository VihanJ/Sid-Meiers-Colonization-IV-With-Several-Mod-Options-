...
void CvTeam::doTurn()
{
	...
	for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
	{
		...
		if (kTeamPlayer.isAlive() && kTeamPlayer.getTeam() == getID())
		{
			...
			{
				if (getRebelPercent() >= GC.getHandicapInfo(getHandicapType()).getKingDowPercent())
				{
					if (GC.getGameINLINE().getSorenRandNum(100, "") == 1)
					{
                        doKingDowRevolution();
					}
				}
	...
	for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
	{
		...
		if (kPlayer.isAlive() && kPlayer.getTeam() == getID() && kPlayer.getParent() != NO_PLAYER && GET_PLAYER(kPlayer.getParent()).isAlive())
		{
			...
			for (CvUnit* pLoopUnit = kPlayer.firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = kPlayer.nextUnit(&iLoop))
			{
				...
				{
					iRebelSentiment += pLoopUnit->getToryValue();
					iCount++;
				}
			}
			// PatchMod: Tories END
	...
	for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
	{
		...
		if (kTeamPlayer.isAlive() && kTeamPlayer.getTeam() == getID())
		{
			if (kTeamPlayer.getParent() != NO_PLAYER)
			{
				...
				if (iCount > 0)
				{
					CvUnit* pGalleonRevolutionUnit = NULL;
					CvPlot* pOceanPlot = NULL;
					while (iCount > 0)
					{
						pOceanPlot = kParent.AI_getImperialShipSpawnPlot();
						pGalleonRevolutionUnit = kParent.initUnit((UnitTypes)35, NO_PROFESSION, pOceanPlot->getX_INLINE(), pOceanPlot->getY_INLINE());
						pGalleonRevolutionUnit->setUnitTravelState(UNIT_TRAVEL_STATE_IN_EUROPE, false);
						iCount = iCount - 6;
					}
				}
				// PatchMod: Send MoWs to New World END
				...
				CvString szTextVal;
				szTextVal = GC.getCivilizationInfo(kTeamPlayer.getCivilizationType()).getRevArtDefineTag();
				GC.getCivilizationInfo(kTeamPlayer.getCivilizationType()).setArtDefineTag(szTextVal);
				int iLoop;
				for (CvUnit* pLoopUnit = kTeamPlayer.firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = kTeamPlayer.nextUnit(&iLoop))
				{
					pLoopUnit->plot()->setFlagDirty(true);
				}
				// Dale - AoD: Rev Flags END

				kTeamPlayer.clearRevolutionEuropeUnits();
				kTeamPlayer.setYieldEuropeTradableAll();
				kTeamPlayer.setTaxRate(25);
				kTeamPlayer.doEra();
...
int CvTeam::getTotalGoodsTraded() const
{
	int iGoodsTraded = 0;
	int iPlayerGoods = 0;
	for (int i = 0; i < MAX_PLAYERS; ++i)
	{
		...
		{
			iPlayerGoods = kPlayer.getTotalGoodsTradedVictory();
			if (!kPlayer.isHuman())
			{
				iPlayerGoods *= GC.getHandicapInfo(getHandicapType()).getAIAdvancedStartPercent();
				iPlayerGoods /= 100;
				iPlayerGoods *= 3;
			}
			iGoodsTraded += iPlayerGoods;
		}
	}
...
int CvTeam::getTotalPiratePlunder() const
{
	int iPiratePlunder = 0;
	int iPlayerPlunder = 0;
	for (int i = 0; i < MAX_PLAYERS; ++i)
	{
		CvPlayer& kPlayer = GET_PLAYER((PlayerTypes) i);
		if (kPlayer.isAlive() && kPlayer.getTeam() == getID() && kPlayer.isPirates())
		{
			iPlayerPlunder = kPlayer.getTotalPiratePlunder();
			if (!kPlayer.isHuman())
			{
				iPlayerPlunder *= GC.getHandicapInfo(getHandicapType()).getAIAdvancedStartPercent();
				iPlayerPlunder /= 100;
				iPlayerPlunder *= 3;
			}
			iPiratePlunder += iPlayerPlunder;
		}
	}

	return iPiratePlunder;
}
// PatchMod: Victorys END

// PatchMod: King DoW START
void CvTeam::doKingDowRevolution()
{
	if (!canDoRevolution())
	{
		return;
	...
	for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
	{
		...
		if (kTeamPlayer.isAlive() && kTeamPlayer.getTeam() == getID())
		{
			if (kTeamPlayer.getParent() != NO_PLAYER)
			{
				...
				pInfo->setData1((CutsceneTypes)1);
				pInfo->setData2(0);

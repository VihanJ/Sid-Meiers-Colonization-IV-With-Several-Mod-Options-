...
void CvGameTextMgr::setUnitHelp(CvWStringBuffer &szString, const CvUnit* pUnit, bool bOneLine, bool bShort)
{
	...
	if (pUnit->getToryValue() >= 50)
	{
		szTempBuffer = NEWLINE + gDLL->getText("TXT_KEY_AODREBEL", pUnit->getToryValue());
		szString.append(szTempBuffer);
	} else if (pUnit->getToryValue() >= 0) {
		szTempBuffer = NEWLINE + gDLL->getText("TXT_KEY_AODTORY", pUnit->getToryValue());
		szString.append(szTempBuffer);
	}
	// PatchMod: Tories END
...
void CvGameTextMgr::setCitizenHelp(CvWStringBuffer &szString, const CvCity& kCity, const CvUnit& kUnit)
{
	...
	// PatchMod: Tories START
	CvWString szTempBuffer;
	if (kUnit.getToryValue() >= 50)
	{
		szTempBuffer = NEWLINE + gDLL->getText("TXT_KEY_AODREBEL", kUnit.getToryValue());
		szString.append(szTempBuffer);
	} else if (kUnit.getToryValue() >= 0) {
		szTempBuffer = NEWLINE + gDLL->getText("TXT_KEY_AODTORY", kUnit.getToryValue());
		szString.append(szTempBuffer);
	}
	// PatchMod: Tories END
	...
	{
		szTempBuffer = NEWLINE + gDLL->getText("TXT_KEY_UNITFREEDOM", kUnit.getTurnsToFreedom());
		szString.append(szTempBuffer);
	}
	// PatchMod: Freedom END
	...
	if (NO_PROFESSION != eProfession)
	{
		...
		if (NO_YIELD != eProfessionYield)
		{
			...
			if (eProfessionYield == YIELD_EDUCATION)
			{
				...
				if (kUnit.getLearningProfession() > 0)
				{
					szTempBuffer = NEWLINE + gDLL->getText("TXT_KEY_UNITLEARNING", GC.getUnitInfo((UnitTypes)kUnit.getLearningProfession()).getDescription());
					szString.append(szTempBuffer);
				}

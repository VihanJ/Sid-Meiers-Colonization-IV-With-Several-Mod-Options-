##############################################################################
## File: Unlimited_World.py	version 2.00
## Author: M07
##
## PURPOSE: CONTINENT map script - Random map that simulates earth-like plate tectonics, geostrophic and monsoon winds and rainfall.
## Copyright 2013 Fratelli Marconi
##############################################################################
##
##############################################################################
## DOANE Version History			( look for #DOANE)
## 2.00-  DOANE: Fix some errors, customize a lot of feature.
## Peaks and hills are well disposed and toundra plots are only at the extreme north or south
## March plots and jungles are well=balanced 
##
## 1.06-  DOANE: Added custom map size options: continental & Planetary.
##Fixed an indentation error which leaded to some crashes.
##
##
##############################################################################
## File: FaireWeatherTweak.py	version 1.05
...
################################################################################
## Version History
## 1.05 -  Added Regularity custom option that allow the user to make the map
##from Very Irregular to regular through 3 levels. Some tweaks in global variables
##to make the maps even better. 
##
...
## GLOBAL TUNING VARIABLES: Change these to customize the map results
		...
		# We can't less than %number plots with the type Peaks
		self.MaxNearPeaks = 5
		
		# We can't less than %number plots with the type Peaks or hills
		self.MaxNearHills = 5
		...
		self.HillChanceAtOne = .007
		self.PeakChanceAtOne = .002
		...
		# Affect the temperature with the latitude of map
		self.PercentLatitudeTempEffect = 1.1

		# Affect the temperature altitude penrcent
		self.AltitudeAboveSeaLevePercent = 0.33
		
		# Affect the temperature with the longitude of map in land
		self.AboveSeaLevelTempEffect = 0.0
		
		# Affect the temperature with the longitude of map in water
		self.NotAboveSeaLevelTempEffect = 0.0
		...
		self.northWaterBand = 6
		self.southWaterBand = 6
		self.eastWaterBand = 28
		self.westWaterBand = 28
		...
		#be optional.
		self.UsePythonRandom = True #DOANE Ensure Multyplayer compatibility ?      
	...
	def initInGameOptions(self):
		...
		gc.setVictory(selectionID, True)
		return
	...
	def createClimateMaps(self):
		...
		for y in range(mc.hmHeight):
			for x in range(mc.hmWidth):
				...
				self.averageTempMap.append(avgTemp * (1.0 - hm.getAltitudeAboveSeaLevel(x,y) * mc.AltitudeAboveSeaLevePercent))
...
##        self.printRainFallMap(False)
	...
	def getInitialTemp(self,x,y,tropic):
		...
		if aboveSeaLevel :
			...
			temp = 1.0 - (tempPerLatChange * latDifference * mc.AboveSeaLevelTempEffect)
		else:
		 	tempPerLatChange = (1.0 - (1.0*mc.oceanTempClamp))/latRange
		 	temp = 1.0 - ( mc.oceanTempClamp - (tempPerLatChange * latDifference)) * mc.NotAboveSeaLevelTempEffect
		temp -= (temp*abs(lat)/90)*mc.PercentLatitudeTempEffect
		return temp
			...
			for x in range(mc.width):
				...
				if self.plotMap[i] == mc.HILLS:
					...
					for direction in range(1,9,1):
						xx,yy = GetXYFromDirection(x,y,direction)
						ii = GetIndex(xx,yy)
						if self.plotMap[ii] != mc.HILLS and self.plotMap[ii] != mc.PEAK:
							iCount += 1
					if iCount < mc.MaxNearHills:
						self.plotMap[i] = mc.LAND
...
def getDescription():
	...
	"""
	return "TXT_KEY_MAP_DOANE_DESCR" 
...
def getCustomMapOptionName(argsList):
		...
		optionID = argsList[0]
		if optionID == 0:
			return localText.getText("TXT_KEY_MAP_SCRIPT_VICTORY_TYPE", ())
		# elif optionID == 1:
		# 	return localText.getText("TXT_KEY_MAP_SCRIPT_LANDFORM", ())
		# elif optionID == 2:
		# 	return localText.getText("TXT_KEY_MAP_SCRIPT_REGULARITY", ())
...
def getCustomMapOptionDescAt(argsList):
	...
	if optionID == 0:
		...
		for i in range(gc.getNumVictoryInfos()):
			if (not gc.getVictoryInfo(i).isPermanent()):
				if(selectionID == iCount):
					szText = gc.getVictoryInfo(i).getDescription()
					if(gc.getVictoryInfo(i).getDefault()):
						 szText +=  u" " + localText.getText("TXT_KEY_MAP_SCRIPT_DEFAULTS", ())
					return szText
				iCount += 1

		if selectionID == 0:
			return localText.getText("TXT_KEY_MAP_SCRIPT_TILES", (3, ))
		elif selectionID == 1:
			return localText.getText("TXT_KEY_MAP_SCRIPT_TILES", (4, ))
		elif selectionID == 2:
			return localText.getText("TXT_KEY_MAP_SCRIPT_TILES", (5, ))
	# elif optionID == 1:
	# 	if selectionID == 0:
	# 		return localText.getText("TXT_KEY_MAP_SCRIPT_LESS_ISLANDS", ())
	# 	elif selectionID == 1:
	# 		return localText.getText("TXT_KEY_MAP_SCRIPT_MORE_ISLANDS", ())
	# elif optionID == 2:
	# 	if selectionID == 0:
	# 		return localText.getText("TXT_KEY_MAP_SCRIPT_VERY_IRREGULAR", ())
	# 	elif selectionID == 1:
	# 		return localText.getText("TXT_KEY_MAP_SCRIPT_QUITE_IRREGULAR", ()) + u" " + localText.getText("TXT_KEY_MAP_SCRIPT_DEFAULTS", ())
	# 	elif selectionID == 2:
	# 		return localText.getText("TXT_KEY_MAP_SCRIPT_REGULAR", ())
	return u""
...
def getGridSize(argsList):
	...
	grid_sizes = {
			...
			#DOANE Worldsize Types
			WorldSizeTypes.WORLDSIZE_CONTINENTAL:	(60 + mc.distanceToEurope * 2,120),
			WorldSizeTypes.WORLDSIZE_PLANETARY:		(70 + mc.distanceToEurope * 2,140)
			#END DOANE
	}
				...
				if sm.rainFallMap[i] > sm.plainsThreshold*1.5:#jungle
					...
					if sm.rainFallMap[i] > PRand.random() * sm.plainsThreshold * 1.5:
						if sm.averageTempMap[i] > mc.ForestTemp:
							plot.setFeatureType(featureForest,FORESTLEAFY) #DOANE fix
						elif sm.averageTempMap[i] > mc.TundraTemp:
							plot.setFeatureType(featureForest,FORESTEVERGREEN)
...
def createIce():
	...
	for y in range(2):
		...
		iceChance *= .66
	iceChance = 1.0
	for y in range(mc.height - 1,mc.height - 3,-1):
		for x in range(mc.width):
			plot = mmap.plot(x,y)
			if plot != 0 and plot.isWater() == True and PRand.random() < iceChance:
				plot.setFeatureType(featureIce,0)

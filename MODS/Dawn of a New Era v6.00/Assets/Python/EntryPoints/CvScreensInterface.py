...
import CvTradeRouteScreen
import CvEuropeC0Screen
import CvEuropeC1Screen
import CvEuropeC2Screen
import CvEuropeC3Screen
...
import CvDawnOfMan
import CvNativeYieldTrade
import CvIncomeAndExpendituresRecord
import CvNewEras
import CvInfoScreen
...
tradeRouteScreen = CvTradeRouteScreen.CvTradeRouteScreen()
def showTradeRouteScreen(argsList):
	if (-1 != CyGame().getActivePlayer()):
		tradeRouteScreen.interfaceScreen(argsList[0])

def setCurrentTradeRouteId(argsList):
	if (-1 != CyGame().getActivePlayer()):
		tradeRouteScreen.setCurrentTradeRouteId(argsList[0])

def setSelectedYield(argsList):
	if (-1 != CyGame().getActivePlayer()):
		tradeRouteScreen.setSelectedYield(argsList[0], argsList[1])

def setLastGroupAssigned(argsList):
	if (-1 != CyGame().getActivePlayer()):
		tradeRouteScreen.setLastGroupAssigned(argsList[0])

europeC0Screen = CvEuropeC0Screen.CvEuropeC0Screen()
def showEuropeScreen(argsList):
	if (-1 != CyGame().getActivePlayer()):
		europeC0Screen.interfaceScreen()

europeC1Screen = CvEuropeC1Screen.CvEuropeC1Screen()
def showEuropeC1Screen(argsList):
	if (-1 != CyGame().getActivePlayer()):
		europeC1Screen.interfaceScreen()
		
europeC2Screen = CvEuropeC2Screen.CvEuropeC2Screen()
def showEuropeC2Screen(argsList):
	if (-1 != CyGame().getActivePlayer()):
		europeC2Screen.interfaceScreen()
		
europeC3Screen = CvEuropeC3Screen.CvEuropeC3Screen()
def showEuropeC3Screen(argsList):
	if (-1 != CyGame().getActivePlayer()):
		europeC3Screen.interfaceScreen()
		
nativeYieldTrade = CvNativeYieldTrade.CvNativeYieldTrade()
def showNativeYieldTrade(argsList):
	if (-1 != CyGame().getActivePlayer()):
		nativeYieldTrade.interfaceScreen(argsList[0], argsList[1])
		
incomeAndExpendituresRecord = CvIncomeAndExpendituresRecord.CvIncomeAndExpendituresRecord()
def showIncomeAndExpendituresRecord(argsList):
	if (-1 != CyGame().getActivePlayer()):
		incomeAndExpendituresRecord.interfaceScreen()

newEras = CvNewEras.CvNewEras()
def showNewEras(argsList):
	if (-1 != CyGame().getActivePlayer()):
		newEras.interfaceScreen()
...
def tradeRoutesOnClickedCallback(argsList):
	iButtonId = argsList[0]
	routeID = argsList[1]
	eTrade = argsList[2]
	iYield = argsList[3]
	...
	if (iButtonId == 0):
		CyMessageControl().sendPlayerAction(CyGlobalContext().getGame().getActivePlayer(), PlayerActionTypes.PLAYER_ACTION_CHANGE_TRADE_TYPE, routeID, eTrade, iYield)
...
#######################################################################################
HandleMouseoverMap = {  EUROPE_SCREEN : europeC0Screen,						
						MAIN_INTERFACE : mainInterface,
						DOMESTIC_ADVISOR : domesticAdvisor,
						MILITARY_ADVISOR : militaryAdvisor,
						REVOLUTION_ADVISOR : revolutionAdvisor,
				# add new screens here
						TRADE_ROUTE_SCREEN : tradeRouteScreen,
						EUROPE_C1_SCREEN : europeC1Screen,
						EUROPE_C2_SCREEN : europeC2Screen,
						EUROPE_C3_SCREEN : europeC3Screen,
						NATIVE_YIELD_TRADE : nativeYieldTrade,
						NEW_ERAS_SCREEN : newEras,
						INCOME_AND_EXPENDITURES_RECORD : incomeAndExpendituresRecord,

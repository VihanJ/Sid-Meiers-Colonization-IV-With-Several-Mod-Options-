...
class CvEventManager:
	def __init__(self):
		...
		self.Events={
			...
			#DOANE: AoDCheatMenu 
			CvUtil.EventAoDCheatMenu: ('AoDCheatMenu', self.__eventAoDCheatMenuApply, self.__eventAoDCheatMenuBegin),
			#END DOANE
			CvUtil.EventEditSeawayName: ('EditSeawayName', self.__eventEditSeawayNameApply, self.__eventEditSeawayNameBegin),
...
#################### ON EVENTS ######################
	...
	def onModNetMessage(self, argsList):
		...
		if(iData1 == self.TERRITORIAL_INFLUENCE_MODE):
			# print("%d, %d, %d, %d, %d", (iData1, iData2, iData3, iData4, iData5))
			pPlot = CyMap().plot(iData4, iData5)
			pPlot.setNotCulture(iData2, iData3)
		elif(iData1 == self.EDIT_SEAWAY_NAME):
			kPlayer =  gc.getPlayer(iData2)
			if (iData2 == gc.getGame().getActivePlayer()):
				if iData3 != -1:
					seaway = kPlayer.getSeaway(iData3)
				else : 
					seaway = kPlayer.getSeawayByID(iData4)
				self.__eventEditSeawayNameBegin(seaway, True)
		elif(iData1 == self.CREATE_SEAWAY):
			kPlayer =  gc.getPlayer(iData2)
			if (iData2 == gc.getGame().getActivePlayer()):
				self.__eventCreateSeawayBegin(iData3, iData4)
...
#################### TRIGGERED EVENTS ##################
	...
	def __eventEditSeawayNameBegin(self, seaway, bRename):
		popup = CyPopup(CvUtil.EventEditSeawayName, EventContextTypes.EVENTCONTEXT_ALL, True)
		popup.setUserData((seaway.getID(), bRename))
		popup.setHeaderString(localText.getText("TXT_KEY_NAME_SEAWAY_HEADER", ()), CvUtil.FONT_CENTER_JUSTIFY)
		popup.setBodyString(localText.getText("TXT_KEY_NAME_SEAWAY", ()), CvUtil.FONT_CENTER_JUSTIFY)
		popup.createEditBox(seaway.getName(), 0)
		popup.setEditBoxMaxCharCount( 19, 26, 0 )
		popup.launch(true, PopupStates.POPUPSTATE_IMMEDIATE)

	def __eventEditSeawayNameApply(self, playerID, userData, popupReturn):

		'Edit City Name Event'
		seawayID = userData[0]
		bRename = userData[1]
		player = gc.getPlayer(playerID)
		seaway = player.getSeawayByID(seawayID)
		seawayName = popupReturn.getEditBoxString(0)
		if (len(seawayName) > 26):
			seawayName = seawayName[:26]
		seaway.setName(seawayName)
		if( gc.getGame().getActivePlayer() == playerID) :
			CyInterface().setDirty(InterfaceDirtyBits.EuropeScreen_DIRTY_BIT, True)
			CyInterface().setDirty(InterfaceDirtyBits.TradeRoutes_DIRTY_BIT, True)
	...
	def __eventCreateSeawayApply(self, playerID, userData, popupReturn):
		...
		if iButton == 0:
			...
			player.addSeaway(plot, seawayName)
	...
	def __eventEditUnitNameApply(self, playerID, userData, popupReturn):
		...
		if( gc.getGame().getActivePlayer() == playerID) :
			CyInterface().setDirty(InterfaceDirtyBits.EuropeC1Screen_DIRTY_BIT, True)
	...
	# DOANE: AoDCheatMenu
	def __eventAoDCheatMenuBegin(self, argsList):
		#Changed the way texts are being adquired to automatically load the AI Autoplay or money value from our global defines.
		iGoldSecond = gc.getDefineINT("CHEAT_MENU_MONEYTREE_SECOND")
		iTurnsFirst = gc.getDefineINT("CHEAT_MENU_AI_AUTOPLAY_FIRST")
		iTurnsSecond = gc.getDefineINT("CHEAT_MENU_AI_AUTOPLAY_SECOND")
		...
		popup.addButton(localText.getText("TXT_KEY_CHEATMENU_DEBUG_MODE", ()))
		popup.addSeparator()

		iOwner = gc.getGame().getActivePlayer()
		popup.setBodyString("Set active player", CvUtil.FONT_CENTER_JUSTIFY)
		...
		for i in range(gc.getMAX_PLAYERS()):
			...
			if (kPlayer.isEverAlive() and not kPlayer.isNative() and not kPlayer.isEurope()):
				if (i != iOwner):
		...
		popup.createSpinBox(0, "", 0, 5, 600, 0) #300
		popup.addButton(localText.getText("TXT_KEY_AIAUTOPLAY_INPUT", (iTurnsFirst,)))
		popup.addButton(localText.getText("TXT_KEY_AIAUTOPLAY_INPUT", (iTurnsSecond,)))
		popup.addSeparator()
		popup.setBodyString(localText.getText("TXT_KEY_MONEYTREE", ()), CvUtil.FONT_CENTER_JUSTIFY)
		popup.createSpinBox(1, "", 0, 1000, 50000, 0) #10000
		popup.addButton(localText.getText("TXT_KEY_MONEYTREE_INPUT", (iGoldSecond,)))
		popup.addSeparator()
		popup.addSeparator()
		popup.launch(True, PopupStates.POPUPSTATE_IMMEDIATE)
		return

	def __eventAoDCheatMenuApply(self, playerID, userData, popupReturn):
		#Added the money and AI Autoplay turns selection as global defines, to easily change values without changing all.
		autoIdx = popupReturn.getButtonClicked()
		iPlayer = gc.getPlayer(playerID)
		iAutoplay = 0
		iAutoplay = popupReturn.getSpinnerWidgetValue(int(0))
		if (autoIdx == 0):
			CyMessageControl().sendPlayerAction(playerID, PlayerActionTypes.PLAYER_ACTION_PERMUTE_DEBUG_MODE, -1, 1, -1);
			return
		if (iAutoplay > 0):
			CyGame().setAIAutoPlay(iAutoplay)
		if (autoIdx == 1):
			iTurns = gc.getDefineINT("CHEAT_MENU_AI_AUTOPLAY_FIRST")
			CyGame().setAIAutoPlay(iTurns) #Autoplay turns 1
		if (autoIdx == 2):
			iTurns = gc.getDefineINT("CHEAT_MENU_AI_AUTOPLAY_SECOND")
			CyGame().setAIAutoPlay(iTurns) #Autoplay turns 2
		iMoneyTree = 0
		iMoneyTree = popupReturn.getSpinnerWidgetValue(int(1))
		if (iMoneyTree > 0):
			iPlayer.changeGold(iMoneyTree)
		if (autoIdx == 3):
			iGold = gc.getDefineINT("CHEAT_MENU_MONEYTREE_FIRST")
			iPlayer.changeGold(iGold) #Money Ammount 1
		if (autoIdx == 4):
			iGold = gc.getDefineINT("CHEAT_MENU_MONEYTREE_SECOND")
			iPlayer.changeGold(iGold) #Money Ammount 2
		playerID = popupReturn.getSelectedPullDownValue( 0 )
		CyGame().setActivePlayer(playerID, True)
		return
	# END DOANE

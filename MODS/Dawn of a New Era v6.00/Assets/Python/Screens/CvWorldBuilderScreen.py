...
#					return []
	...
	def handleUnitProfessionEditPullDownCB ( self, argsList ) :
		...
		for i in range(gc.getNumProfessionInfos()):
			if not gc.getProfessionInfo(i).isCitizen():
				if pPlayer.isProfessionValid(i, pUnit.getUnitType()):
					if iNewProfession == int(argsList[0]):
						pUnit.setProfession(i)
						return 1
					iNewProfession += 1
		return 0
			...
			if gc.getUnitInfo(iUnitType).isTreasure():
				...
				for i in range(YieldTypes.NUM_YIELD_TYPES):
					if gc.getYieldInfo(i).getUnitClass() == gc.getUnitInfo(iUnitType).getUnitClassType():
						iYieldAmount = gc.getGame().getCargoYieldCapacity()
						if (self.m_pCurrentPlot.isCity()):
							self.m_pCurrentPlot.getPlotCity().changeYieldStored(i, iYieldAmount)
							makeUnit = false
						if makeUnit:
							for iLoopUnit in range (self.m_pCurrentPlot.getNumUnits()):
								pUnit = self.m_pCurrentPlot.getUnit(iLoopUnit)
								iSpace = pUnit.getSpaceNewCargo()
								iAmount = min(iSpace, iYieldAmount)
								if(iSpace > 0) :
									pUnit.changeNewCargoYield(i, iAmount)
									makeUnit = false
						break
			if makeUnit:
				...
				if(iUnitType == UnitTypes.UNIT_SAILOR_EXPERT) :
					pUnit.setCanBeSailor(true)
					pUnit.setProfession(iProfession)
				pUnit.setNbMaxCannon()
				pUnit.setMaxMunition()

			...
			if gc.getGame().isVictoryValid(iLoopVC):
				...
				if (victory.isRevolution()):
					if activePlayer.getParent() != -1:
						...
						bEntriesFound = True
				if (victory.getTargetNewEra() != NewEraTypes.NO_NEW_ERA):
					screen.setTableColumnHeader(szTable, 0, "", self.TABLE_WIDTH_0*3/2)
					iRow = screen.appendTableRow(szTable)
					screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_NEW_ERA", (victory.getTargetNewEra() + 1, )), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)

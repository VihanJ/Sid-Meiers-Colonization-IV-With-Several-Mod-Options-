...
class CvInfoScreen:
	...
	def setGlobals(self):
		...
		self.NUM_OFFSET_TURN = 10 # Number of turns that will be hide for active player in graphics
		...
		self.LAND_MILITARY_SCORE = 4
		self.SEA_MILITARY_SCORE	= 5
		self.NAVAL_MERCHANT_SCORE = 6
		self.CULTURE_SCORE	= 7
		self.NUM_SCORES		= 8
...
############################################### STATISTICS ###############################################
	...
	def initText(self):
		...
		self.TEXT_LAND_MILITARY = localText.getText("TXT_KEY_LAND_MILITARY", ())
		self.TEXT_SEA_MILITARY = localText.getText("TXT_KEY_SEA_MILITARY", ())
		self.TEXT_NAVAL_MERCHANT = localText.getText("TXT_KEY_NAVAL_MERCHANT", ())
...
#		self.bShowAllPlayers = false
	...
	def showScreen(self, iTurn, iTabID, iEndGame):
		...
		hasWinner = CyGame().getWinner() != TeamTypes.NO_TEAM;
		for iLoopPlayer in range(gc.getMAX_CIV_PLAYERS()):
			...
			if (gc.getTeam(iLoopPlayerTeam).isEverAlive()):
				if ((self.pActiveTeam.isHasMet(iLoopPlayerTeam) or hasWinner) and not gc.getPlayer(iLoopPlayer).isEurope() and not gc.getPlayer(iLoopPlayer).isNative()):
...
#################################################### GRAPH ##################################################
	...
	def drawPermanentGraphWidgets(self):
		...
		screen.addPullDownString(self.szGraphDropdownWidget, self.TEXT_LAND_MILITARY, 4, 4, False )
		screen.addPullDownString(self.szGraphDropdownWidget, self.TEXT_SEA_MILITARY, 5, 5, False )
		screen.addPullDownString(self.szGraphDropdownWidget, self.TEXT_NAVAL_MERCHANT, 6, 6, False )
		screen.addPullDownString(self.szGraphDropdownWidget, self.TEXT_CULTURE, 7, 7, False )
	...
	def computeHistory(self, scoreType, iPlayer, iTurn):
		...
		elif (scoreType == self.LAND_MILITARY_SCORE):
			return gc.getPlayer(iPlayer).getLandMilitaryHistory(iTurn)
		elif (scoreType == self.SEA_MILITARY_SCORE):
			return gc.getPlayer(iPlayer).getSeaMilitaryHistory(iTurn)
		elif (scoreType == self.NAVAL_MERCHANT_SCORE):
			return gc.getPlayer(iPlayer).getNavalMerchantHistory(iTurn)
	...
	def drawGraphYLabels(self, iMin, iMax):
		...
		for iLabelPosition in range(iNumLabels):
			...
			screen.setLabel( self.getNextWidgetName(), "", u"<font=2>%d</font>"%(iValue), CvUtil.FONT_RIGHT_JUSTIFY, xLabel, yPosition, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
			if(iLabelPosition > 0):
				screen.addDDSGFC(self.getNextWidgetName(), ArtFileMgr.getInterfaceArtInfo("INTERFACE_TRADE_ROUTES_SMALL_LINE").getPath(), self.X_LEFT_LABEL - 50, yPosition + 14, self.W_GRAPH, 3,  WidgetTypes.WIDGET_GENERAL, -1, -1 )
	...
	def drawGraph(self):
		...
		for p in self.aiPlayersMet:
			...
			if (self.aiPlayerToggle[p]):
				loopPlayer = gc.getPlayer(p)
				color = gc.getPlayerColorInfo(loopPlayer.getPlayerColor()).getColorTypePrimary()
				oldX = -1
				oldY = self.H_GRAPH
				turn = lastTurn
				if(loopPlayer.getTeam() != eTeam and CyGame().getWinner() == TeamTypes.NO_TEAM) :
					turn -= self.NUM_OFFSET_TURN # We hide the last turns for active player to avoid spying 
...
################################################# DEMOGRAPHICS ##############################################
	...
	def drawTextChart(self):
		...
		for iPlayerLoop in range(gc.getMAX_PLAYERS()):
			...
			if (pPlayer.isAlive() and not pPlayer.isNative() and not pPlayer.isEurope()):
...
################################################## TOP CITIES ###############################################
	...
	def drawTopCities(self):
		...
		for iWidgetLoop in range(self.iNumCities):
			...
			if (pCity.isRevealed(gc.getGame().getActiveTeam(), false)):
				...
				screen.addDDSGFC(self.getNextWidgetName(), ArtFileMgr.getInterfaceArtInfo("INTERFACE_TOP_COLONIES_UNKNOWN_COLONY").getPath(), self.X_CITY_ANIMATION, self.Y_ROWS_CITIES[iWidgetLoop], self.W_CITY_ANIMATION, self.H_CITY_ANIMATION, WidgetTypes.WIDGET_GENERAL, -1, -1 )
	...
	def calculateTopCities(self):
		...
		for iPlayerLoop in range(gc.getMAX_PLAYERS()):
			...
			if not player.isNative():

				(pCity, iter) = player.firstCity(false)

				while(pCity):

					iTotalCityValue = pCity.getPopulation() * 1000 + pCity.getYieldRate(YieldTypes.YIELD_FOOD) 
		...
		if (code == NotifyCode.NOTIFY_LISTBOX_ITEM_SELECTED):
			...
			if (inputClass.getFunctionName() == self.DEBUG_DROPDOWN_ID):
				...
				for iLoopPlayer in range(gc.getMAX_CIV_PLAYERS()):
					pLoopPlayer = gc.getPlayer(iLoopPlayer)
					iLoopPlayerTeam = pLoopPlayer.getTeam()
					if (self.pActiveTeam.isHasMet(iLoopPlayerTeam) or hasWinner):
						self.aiPlayersMet.append(iLoopPlayer)
						self.iNumPlayersMet += 1
				self.redrawContents()
			...
			if (self.iActiveTab == self.iGraphID):
				...
				if (szWidgetName == self.szGraphDropdownWidget):
					...
					if (iSelected == 0):
						...
						self.iGraphTabID = self.LAND_MILITARY_SCORE

					elif (iSelected == 5):
						self.iGraphTabID = self.SEA_MILITARY_SCORE

					elif (iSelected == 6):
						self.iGraphTabID = self.NAVAL_MERCHANT_SCORE

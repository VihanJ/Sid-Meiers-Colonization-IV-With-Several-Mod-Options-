...
class CvPediaBonus:
	...
	def placeStats(self):
		...
		##DOANE Resources info: add new info for all resources related with the "Seed system"
		#Get Latitude values for all bonuses
		iYieldMinLat = gc.getBonusInfo(self.iBonus).getMinLatitude()
		iYieldMaxLat = gc.getBonusInfo(self.iBonus).getMaxLatitude()
		
		#Seed System Bonuses
		if (gc.getBonusInfo(self.iBonus).isSeed()):
			#Get Ideal Latitude values
			iYieldMinIdealLat = gc.getBonusInfo(self.iBonus).getMinIdealLatitude()
			iYieldMaxIdealLat = gc.getBonusInfo(self.iBonus).getMaxIdealLatitude()
			#Latitude range
			szYieldLat = (u"Latitude: %i - %i" % (iYieldMinLat, iYieldMaxLat))
			screen.appendListBoxString(panelName, u"<font=3>" + szYieldLat + u"</font>", WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)
			#Ideal latitude range
			szYieldIdealLat = (u"Ideal latitude: %i - %i" % (iYieldMinIdealLat, iYieldMaxIdealLat))
			screen.appendListBoxString(panelName, u"<font=3>" + szYieldIdealLat + u"</font>", WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)
			#Required terrains for plantations
			szYieldTerrain = u"<font=4>" + "Plantation:" + u"</font>"
			iCount = 0;
			for k in range(gc.getNumTerrainInfos()):
				#Terrain
				if (gc.getBonusInfo(self.iBonus).isTerrain(k)):
					if (iCount): szYieldTerrain += ", " #This puts commas after all values but first and last...
					if (iCount%2 == 0): szYieldTerrain += u"\n" #Adds a new line when 2 terrains have been added to the text
					szYieldTerrain += gc.getTerrainInfo(k).getDescription() 
					iCount += 1
				#Features in Terrain
				elif (gc.getBonusInfo(self.iBonus).isFeatureTerrain(k)):
					for j in range(gc.getNumFeatureInfos()):
						if (gc.getBonusInfo(self.iBonus).isFeature(j)):
							if (iCount): szYieldTerrain += ", " #This puts commas after all values but first and last...
							szYieldTerrain += u"\n" #This always needs a new line
							szYieldTerrain += "%s (in %s)" % (gc.getFeatureInfo(j).getDescription(),  gc.getTerrainInfo(k).getDescription())
							iCount += iCount%2 #This assures a new line will always be created  after this text...
			screen.appendListBoxString(panelName, u"<font=3>" + szYieldTerrain + u"</font>", WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)

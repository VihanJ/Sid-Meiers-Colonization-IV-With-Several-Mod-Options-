...
FOREIGN_EUROPE_TRADE_SCREEN = 2
NUM_FOREIGN_SCREENS = 3
...
class CvForeignAdvisor:
	...
	def __init__(self):
		...
		self.CHANGE_SELECTED_YIELD = 1
		self.CHANGE_SELECTED_YIELD_NO_TRADE = 2
	...
	def interfaceScreen (self, iScreen):
		...
		self.X_YIELDS_PANEL = self.xSize(50)
		self.Y_YIELDS_PANEL = self.ySize(70)
		self.H_YIELDS_PANEL = self.ySize(400)
		self.W_YIELDS_PANEL = self.W_LEGEND

		self.H_TRADE_LEGEND = self.ySize(100)
	...
	def drawContents(self, bInitial):
		...
		if (self.iScreen == FOREIGN_RELATIONS_SCREEN):
			...
			self.drawActive()
		elif (self.iScreen == FOREIGN_EUROPE_TRADE_SCREEN):
			self.drawEuropeTrades()
		self.drawTabs()

	def drawEuropeTrades(self):
		screen = self.getScreen()
		...
		for iPlayer in range(gc.getMAX_PLAYERS()):
			...
			if (player.isEurope() and player.isAlive()):
				leaderMap[iPlayer] = iPlayerCount
		...
		for iYield in range(YieldTypes.NUM_YIELD_TYPES):
			if gc.getYieldInfo(iYield).isRawMaterial():
				if not playerActive.isHasYieldUnknown(iYield):
					tradeYields[iYield] = yieldCount
					yieldCount = yieldCount + 1
					if self.selectedYield == -1:
						currentValue = 1
						if currentValue > bestValue:
							self.selectedYield = iYield
							bestValue = currentValue
		
		iDecalY = self.ySize(5)
		iSpacingY = self.ySize(10)
		iYieldSize = self.minSize(30)
		iYieldDescX = iYieldSize * 5 / 4
		iYieldDescOffsetY = self.ySize(15)
		panelName = self.getNextWidgetName()
		self.H_YIELDS_PANEL = yieldCount * (iYieldSize + iSpacingY) + iDecalY * 2
		screen.addPanel(panelName, u"", u"", True, False, self.X_YIELDS_PANEL, self.Y_YIELDS_PANEL, self.W_YIELDS_PANEL, self.H_YIELDS_PANEL, PanelStyles.PANEL_STYLE_IN, WidgetTypes.WIDGET_GENERAL, -1, -1)

		iPosY = 0
		for iYield in tradeYields.keys():
			szText = gc.getYieldInfo(iYield).getDescription()
			hasBeenTraded = False
			action = self.CHANGE_SELECTED_YIELD
			for iPlayer in leaderMap.keys():
				player = gc.getPlayer(iPlayer)
				for iLoopPlayer in leaderMap.keys():
					if (iLoopPlayer != iPlayer):
						if player.getEuropeTradeTotalAmount(iYield, iLoopPlayer) > 0:
							hasBeenTraded = True
							break
			if not hasBeenTraded:
				szText = localText.changeTextColor(szText, gc.getInfoTypeForString("COLOR_RED"))
				action = self.CHANGE_SELECTED_YIELD_NO_TRADE
			if self.selectedYield == iYield:
				szText = u"<font=3b>" + szText + u"</font>"
			else:
				szText = u"<font=2>" + szText + u"</font>"
				 
			screen.setImageButtonAt(self.getNextWidgetName(), panelName, gc.getYieldInfo(iYield).getIcon(), 0, tradeYields[iYield] * (iYieldSize + iSpacingY) + iDecalY, iYieldSize, iYieldSize, WidgetTypes.WIDGET_GENERAL, action, iYield)
			screen.setTextAt(self.getNextWidgetName(), panelName, szText, CvUtil.FONT_LEFT_JUSTIFY, iYieldDescX, tradeYields[iYield] * (iYieldSize + iSpacingY) + iDecalY + iYieldDescOffsetY, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, action, iYield)

		circleCenterX = self.XResolution * 3 / 5
		circleCenterY = self.YResolution * 7 / 20
		circleRadius = self.minSize(170)
		initRadian = (-90) * 0.01745329252 
		deltaRadian = (360 / iPlayerCount) * 0.01745329252 
		
		for iPlayer in leaderMap.keys():
			player = gc.getPlayer(iPlayer)

			# Displaying leader icon
			szLeaderHead = self.getNextWidgetName()
			
			iPercent = 0
			iLeaderHeight = self.H_LEADER + self.H_LEADER * iPercent / 100
			iLeaderWidth = self.W_LEADER + self.H_LEADER * iPercent / 100

			playerIconX = int(circleCenterX + circleRadius * math.cos(initRadian + deltaRadian * leaderMap[iPlayer]) - iLeaderWidth / 2)
			playerIconY = int(circleCenterY + circleRadius * math.sin(initRadian + deltaRadian * leaderMap[iPlayer]) - iLeaderHeight / 2)

			screen.addCheckBoxGFC(szLeaderHead, gc.getLeaderHeadInfo(player.getLeaderType()).getButton(), "", playerIconX, playerIconY, iLeaderWidth, iLeaderHeight, WidgetTypes.WIDGET_LEADERHEAD_KING, iPlayer, self.selectedYield, ButtonStyles.BUTTON_STYLE_LABEL)

			# Displaying leader name
			...
			szText = u"<font=2>" + u"<color=%d,%d,%d,%d>%s" %(textColorR, textColorG, textColorB, textColorA, gc.getCivilizationInfo(player.getCivilizationType()).getCivilopedia()) + u"</font>"
			screen.setLabel(szName, "", szText, CvUtil.FONT_CENTER_JUSTIFY, playerIconX + iLeaderWidth/2, playerIconY + iLeaderHeight + 5, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
			
			# display trade relations (lines) 
			for iLoopPlayer in leaderMap.keys():
				if (iLoopPlayer != iPlayer):
					pLoopPlayer = gc.getPlayer(iLoopPlayer)
					loopPlayerIconX = int(circleCenterX + circleRadius * math.cos(initRadian + deltaRadian * leaderMap[iLoopPlayer]) - iLeaderWidth / 2)
					loopPlayerIconY = int(circleCenterY + circleRadius * math.sin(initRadian + deltaRadian * leaderMap[iLoopPlayer]) - iLeaderHeight / 2)
					europeTradeAmount = player.getEuropeTradeTotalAmount(self.selectedYield, iLoopPlayer) - pLoopPlayer.getEuropeTradeTotalAmount(self.selectedYield, iPlayer)
					if europeTradeAmount > 0:
						middlePointX = (loopPlayerIconX + playerIconX) / 2
						middlePointY = (loopPlayerIconY + playerIconY) / 2
						screen.addLineGFC(self.BACKGROUND_ID, self.getNextLineName(), playerIconX + iLeaderWidth / 2, playerIconY + iLeaderHeight / 2, middlePointX + iLeaderWidth / 2, middlePointY + iLeaderHeight / 2, gc.getInfoTypeForString("COLOR_BLUE") )
						screen.addLineGFC(self.BACKGROUND_ID, self.getNextLineName(), middlePointX + iLeaderWidth / 2, middlePointY + iLeaderHeight / 2, loopPlayerIconX + iLeaderWidth / 2, loopPlayerIconY + iLeaderHeight / 2, gc.getInfoTypeForString("COLOR_CITY_GREEN") )
					elif player.isMarketTransactionBlocked(self.selectedYield, iLoopPlayer):
						screen.addLineGFC(self.BACKGROUND_ID, self.getNextLineName(), playerIconX + iLeaderWidth / 2, playerIconY + iLeaderHeight / 2, loopPlayerIconX + iLeaderWidth / 2, loopPlayerIconY + iLeaderHeight / 2, gc.getInfoTypeForString("COLOR_RED") )
					
		# legend
		screen.addPanel(self.getNextWidgetName(), u"", u"", True, False, self.X_YIELDS_PANEL, self.YResolution - self.H_TRADE_LEGEND - 75, self.W_LEGEND, self.H_TRADE_LEGEND, PanelStyles.PANEL_STYLE_IN, WidgetTypes.WIDGET_GENERAL, -1, -1)
		x = self.X_YIELDS_PANEL + self.MARGIN_LEGEND
		y = self.YResolution - self.H_TRADE_LEGEND + self.MARGIN_LEGEND * 2 - 75
		screen.setLabel(self.getNextWidgetName(), "", u"<font=2>" + localText.getText("TXT_KEY_TRADE_IMPORT", ()) + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, x, y-10, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		...
		screen.setLabel(self.getNextWidgetName(), "", u"<font=2>" + localText.getText("TXT_KEY_TRADE_EXPORT", ()) + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, x, y-10, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		y += self.MARGIN_LEGEND
		...
		# Yield Trade statistic
		kSelectedInfo = gc.getYieldInfo(self.selectedYield)
		marginYieldTradeStatPanel = self.minSize(20)
		hRow = self.ySize(30)
		wYieldTradeStatPanel = self.xSize(600)
		xYieldTradeStatPanel = circleCenterX - wYieldTradeStatPanel / 2
		hYieldTradeStatPanel = self.ySize(150)
		yYieldTradeStatPanel = self.YResolution - hYieldTradeStatPanel - 75 + self.MARGIN_LEGEND
		yieldTradeStatPanelName = self.getNextWidgetName()
		screen.addPanel(yieldTradeStatPanelName, u"", u"", True, False, xYieldTradeStatPanel, yYieldTradeStatPanel, wYieldTradeStatPanel, hYieldTradeStatPanel, PanelStyles.PANEL_STYLE_IN, WidgetTypes.WIDGET_GENERAL, -1, -1)
		
		iLastTurnBestPlayer = PlayerTypes.NO_PLAYER
		iLastTurnBestValue = 0
		iLastTurnTotalValue = 0
		iLastXTurnsBestPlayer = PlayerTypes.NO_PLAYER
		iLastXTurnsBestValue = 0
		iLastXTurnsTotalValue = 0
		for iPlayer in leaderMap.keys():
			currentPlayer = gc.getPlayer(iPlayer)
			iLastTurnValue = 0
			iLastXTurnsValue = 0
			for iLoopPlayer in leaderMap.keys():
				if iLoopPlayer == iPlayer:
					continue
				iLastTurnValue += currentPlayer.getEuropeTradeAmount(self.selectedYield, iLoopPlayer)
				iLastXTurnsValue += currentPlayer.getEuropeTradeTotalAmount(self.selectedYield, iLoopPlayer)
			iLastTurnTotalValue += iLastTurnValue
			iLastXTurnsTotalValue += iLastXTurnsValue
			if (iLastTurnValue > iLastTurnBestValue):
				iLastTurnBestValue = iLastTurnValue
				iLastTurnBestPlayer = iPlayer
			if (iLastXTurnsValue > iLastXTurnsBestValue):
				iLastXTurnsBestValue = iLastXTurnsValue
				iLastXTurnsBestPlayer = iPlayer

		# temp hack end
		yPos = 0
		lastTurnBestPlayer = u"-"
		iPercent = 0
		if iLastTurnBestPlayer != PlayerTypes.NO_PLAYER :
			iPercent = iLastTurnBestValue * 100 / iLastTurnTotalValue
			bestPlayer = gc.getPlayer(iLastTurnBestPlayer)
			lastTurnBestPlayer = u"<color=%d,%d,%d,%d>%s</color>" %(bestPlayer.getPlayerTextColorR(), bestPlayer.getPlayerTextColorG(), bestPlayer.getPlayerTextColorB(), bestPlayer.getPlayerTextColorA(), gc.getCivilizationInfo(bestPlayer.getCivilizationType()).getCivilopedia())
		screen.setLabelAt(self.getNextWidgetName(), yieldTradeStatPanelName, u"<font=2>" + localText.getText("TXT_KEY_LAST_TURN_BEST_EXPORTER", (kSelectedInfo.getDescription(), lastTurnBestPlayer, iPercent, )) + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, marginYieldTradeStatPanel, marginYieldTradeStatPanel + yPos * hRow, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
		
		yPos += 1
		lastXTurnsBestPlayer = u"-"
		numTurns = 15
		iPercent = 0
		if iLastXTurnsBestPlayer != PlayerTypes.NO_PLAYER :
			iPercent = iLastXTurnsBestValue * 100 / iLastXTurnsTotalValue
			bestPlayer = gc.getPlayer(iLastXTurnsBestPlayer)
			lastXTurnsBestPlayer = u"<color=%d,%d,%d,%d>%s</color>" %(bestPlayer.getPlayerTextColorR(), bestPlayer.getPlayerTextColorG(), bestPlayer.getPlayerTextColorB(), bestPlayer.getPlayerTextColorA(), gc.getCivilizationInfo(bestPlayer.getCivilizationType()).getCivilopedia())
		screen.setLabelAt(self.getNextWidgetName(), yieldTradeStatPanelName, u"<font=2>" + localText.getText("TXT_KEY_LAST_X_TURNS_BEST_EXPORTER", (kSelectedInfo.getDescription(), lastXTurnsBestPlayer, numTurns, iPercent, )) + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, marginYieldTradeStatPanel, marginYieldTradeStatPanel + yPos * hRow, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
		
		yPos += 1
		iTotalTradeRessourcesForLastTurn = iLastTurnTotalValue
		screen.setLabelAt(self.getNextWidgetName(), yieldTradeStatPanelName, u"<font=2>" + localText.getText("TXT_KEY_LAST_TURN_TOTAL_TRADE", (kSelectedInfo.getDescription(), iTotalTradeRessourcesForLastTurn, kSelectedInfo.getChar(), )) + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, marginYieldTradeStatPanel, marginYieldTradeStatPanel + yPos * hRow, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
		
		yPos += 1
		iTotalTradeRessourcesForLastXTurns = iLastXTurnsTotalValue
		screen.setLabelAt(self.getNextWidgetName(), yieldTradeStatPanelName, u"<font=2>" + localText.getText("TXT_KEY_LAST_X_TURNS_TOTAL_TRADE", (kSelectedInfo.getDescription(), iTotalTradeRessourcesForLastXTurns, kSelectedInfo.getChar(), numTurns)) + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, marginYieldTradeStatPanel, marginYieldTradeStatPanel + yPos * hRow, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
	...
	def drawTabs(self):
		...
		Tabs = [localText.getText("TXT_KEY_FOREIGN_ADVISOR_RELATIONS", ()), localText.getText("TXT_KEY_FOREIGN_ADVISOR_ACTIVE", ()), localText.getText("TXT_KEY_FOREIGN_ADVISOR_EUROPE_TRADE", ())]
		NumTabs = len(Tabs)
	...
	def handleInput (self, inputClass):
		if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED):
			if (inputClass.getButtonType() == WidgetTypes.WIDGET_LEADERHEAD):
				...
				if (inputClass.getData1() == 1111):
					self.interfaceScreen(inputClass.getData2())
				elif (inputClass.getData1() == self.CHANGE_SELECTED_YIELD or inputClass.getData1() == self.CHANGE_SELECTED_YIELD_NO_TRADE):
					self.selectedYield = inputClass.getData2()
					self.drawContents(False)
	...
	def getWidgetHelp(self, argsList):
		...
		if eWidgetType == WidgetTypes.WIDGET_GENERAL:
			if iData1 == self.CHANGE_SELECTED_YIELD_NO_TRADE:
				kYield = gc.getYieldInfo(iData2)
				return localText.getText("TXT_KEY_FOREIGN_ADVISOR_EUROPE_TRADE_NO_TRADE", (kYield.getDescription(), ))
